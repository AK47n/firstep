...
PERIPHERAL_TERMS: tuple[str, ...] = (
    ...
    # 器件类别词（工单 preselect-visibility/05）：每个词项都在参考库有承载条目
    # （类别手册合集）——加词项时先确认有承载条目，否则该词项对关联无效
    "气压", "光照", "颜色", "气体", "测距", "称重", "姿态", "指纹", "语音",
    "触摸", "摇杆", "彩屏", "灯带", "无线数传", "lora", "温度",
    # 存储 / 编码 / 环境类（工单 05 整改：不给这些件用「温度」兜底——存储与
    # 编码件无温度语义，兜底会把它们关联到芯片内部测温例程）
    "存储", "旋转编码", "人体感应", "雷达", "雨滴", "土壤湿度",
)
...
MODULE_PERIPHERAL_TERMS: dict[str, tuple[str, ...]] = {
    ...
    "xunji": ("循迹", "巡线"),
    "k230": ("k230",),
    # 器件类别映射（工单 preselect-visibility/05）：模块 → 类别词项，词项由
    # 「器件手册合集」条目标题承载。词项必须是该器件的真实类别语义——不用宽泛
    # 兜底词（如把存储/编码件挂「温度」会关联到芯片内部测温例程，工单 05 整改）
    "ads1115": ("称重",),
    "ags10": ("气体",),
    "aht10": ("温湿度",),
    "as32": ("无线数传", "lora"),
    ...
    "gp2y1014au": ("气体",),
    "hc05": ("蓝牙", "无线数传"),
    # 磁力计双件（工单 magnetometer-modules/03）：类别词项取既有的「姿态」
    # （姿态传感器器件手册合集-姿态 / MPU6050 姿态解算 DMP 库 / MSPM0 m0imu
    # 姿态例程 三条参考条目）——参考库内无「磁力计」条目（lckfb 无该器件手册
    # 页），故用最贴近的既有类别；两件互为替换件（互替不同选）
    "hmc5883l": ("姿态",),
    "qmc5883l": ("姿态",),
    "human_ir": ("人体感应",),
    "hx711": ("称重",),
