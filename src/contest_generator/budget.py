...
# 词表段变更配套（2026-09-06，wiki-modules-batch13）：词表完整 wire 实测 7374、
# 预算 7500→7600 → 词表段全量 7374 比旧截断形态 7334 多 40B → 全文降 100B 回
# 60300 保 2KB 边界余量（同 batch5/7/8/9/12 口径；红证见 worst-case 结构测试实测）。
# 词表段变更配套（2026-09-06，wiki-modules-batch13）：词表完整 wire 实测 7557、
# 预算 7600→7800 → 词表段全量 7557 比旧截断形态 7434 多 123B → 全文降 100B 回
# 60200 保 2KB 边界余量（同 batch5/7/8/9/12 口径；红证见 worst-case 结构测试实测）。
REFERENCE_FULLTEXT_BYTES = 60200
