...
# 2026-09-06（wiki-modules-batch5）：感知传感器 +3 方案（ADS1115/TCS34725/
# MLX90614）+ 新分类「存储/数据记录」（AT24C02）→ 默认词表完整 wire 实测
# 4925（> 4534 fit 上限 4700−166，尾部类别被截、方案名丢失——选中判决依据
# 折损；旧「实测 4698」系截断形态误记），预算升 5200（fit 上限 5034 ≥ 4925
# 全量送达 + 109B 余量；词表段全量 4925 比截断形态 4700 多 225B，最坏形态
# 总量 +225B——全文预算配套下调 500B 回 62500 保 2KB 边界余量）。
# 2026-09-07（wiki-modules-batch6）：感知传感器 +4 方案（DS18B20/SHT30/MQ-2/
# TTP224）→ 默认词表完整 wire 实测 5201（> 5034 fit 上限 5200−166，尾部
# 类别被截、方案名丢失），预算升 5400（fit 上限 5234 ≥ 5201 全量送达 + 33B
# 余量；词表段全量 5201 比旧截断形态 5200 仅多 1B——最坏形态总量 +1B，全文
# 预算 62500 不动，2KB 边界余量保持）。
# 2026-09-08（wiki-modules-batch7）：感知传感器 +4 方案（MQ-135/MQ-5/SGP30/
# AGS10）→ 默认词表完整 wire 实测 5495（> 5234 fit 上限 5400−166，尾部
# 类别被截、方案名丢失），预算升 5700（fit 上限 5534 ≥ 5495 全量送达 + 39B
# 余量；词表段全量 5495 比旧截断形态 5300 多 195B——最坏形态总量 +195B，
# 全文预算 62500→62000 保 2KB 边界余量（batch5 先例：+225B 降 500B）。
# 2026-09-06（wiki-modules-batch8）：感知传感器 +4 方案（火焰/土壤湿度/
# 人体红外/微波雷达）→ 默认词表完整 wire 实测 5900（> 5534 fit 上限
# 5700−166，尾部类别被截、方案名丢失），预算升 6100（fit 上限 5934 ≥ 5900
# 全量送达 + 34B 余量；词表段全量 5900 比旧截断形态 5700 多 200B——最坏形态
# 总量 +200B，全文预算 62000→61500 保 2KB 边界余量（batch5/7 先例口径）。
# 2026-09-09（wiki-modules-batch9）：感知传感器 +4 方案（光敏电阻/雨滴/
# GP2Y1014AU 粉尘/S12SD 紫外线）→ 默认词表完整 wire 实测 6341（> 5934 fit
# 上限 6100−166，尾部类别被截、方案名丢失——test_wordlist_segment 契约红证），
...
# 2026-09-11（wiki-modules-batch11）：感知传感器 +7 方案（MQ-3/MQ-4/MQ-6/
# MQ-7/MQ-8/MQ-9/MS1100——MQ 系同构快补收尾）→ 默认词表完整 wire 实测 7051
# （> 6634 fit 上限 6800−166，尾部类别被截、方案名丢失——test_wordlist_segment
# 契约红证），预算升 7300（fit 上限 7134 ≥ 7051 全量送达 + 83B 余量；词表段
# 全量 7051 比旧截断形态 6634 多 417B——最坏形态总量 +417B，全文预算
# 61000→60500 保 2KB 边界余量（batch5/7/8/9 口径）。
# 2026-09-12（wiki-modules-batch12）：显示模块 +1 方案（IPS 彩屏六屏合一，
# models +4：ST7735/ST7789/GC9A01/XPT2046）→ 默认词表完整 wire 实测 7239
# （> 7134 fit 上限 7300−166，尾部类别被截、方案名丢失——test_wordlist_segment
# 契约红证），预算升 7500（fit 上限 7334 ≥ 7239 全量送达 + 95B 余量；词表段
# 全量 7239 比旧截断形态 7134 多 105B——最坏形态总量 +105B，全文预算
# 60500→60400 保 2KB 边界余量（batch5/7/8/9 口径）。
# 2026-09-06（wiki-modules-batch13）：执行机构 +1 方案（1 路 5V 继电器）+
# models +1（继电器）；无线通信模块「LoRa 数传」solution 补 as32=串口版 +
# models +1（AS32）；感知传感器 +2 方案（BMP180/MS5611）+ models +2 →
# 默认词表完整 wire 实测 7692（> 7334 fit 上限 7500−166，尾部类别被截、
# 方案名丢失——test_wordlist_segment 契约红证），预算两级上调 7500→7900
# （7600/7800 中间步实测 7374/7557 均超截——见 git 提交记录；fit 上限
# 7734 ≥ 7692 全量送达 + 42B 余量；词表段全量 7692 比旧截断形态 7334 多
# 358B——最坏形态总量 +358B，全文预算 60400→60100（两级 100B）保 2KB 边界
# 余量（batch5/7/8/9/12 口径）。
# 2026-09-07（wiki-stm32-batch7/04/05）：感知传感器 +2 方案（EC11 旋转编码器
# + 4×4 矩阵键盘——B 类新 slug 仅 stm32 条目，wordlist 首次 B 类补录）+
# models +2（EC11 旋转编码器 / 4×4 矩阵键盘）→ 默认词表完整 wire 分两刀实
# 测：ec11 后 7812（> 7734 fit 上限 7900−166，截断红证）预算 7900→8050
# （fit 上限 7884 ≥ 7812 + 72B 余量）；key_matrix 后 7886（> 7884 差 2B
# 截断红证）预算 8050→8120（fit 上限 7954 ≥ 7886 + 68B 余量）；词表段
...
# 2026-09-17（工单 real-acceptance/08）：词表 models 补方案裸名（B1 数据对齐——
# 把「提示词给模型看过的名字」登记进闸认的合法 name 域，模型照抄方案名不再被拒；
# 现场 2022C 连续三轮挂在这一类名字上）→ 词表段实发 8849 → **9619**（+770，
# 60 条备选裸名里收得下 15 条，其余 38 条未入选——裁定与清单见工单 08）。
# 预算升 9200 → 12150：基线 8849 已顶在旧 fit 上限 9034 附近，新增即截断尾部类别
# 方案名（契约用例 test_wordlist_segment_covers_default_wordlist_and_budget 红）。
...
# 域拒绝重试的追加指令（工单 real-acceptance/03；「怎么改」段工单 real-acceptance/11
# 方向 ③）：把被拒理由原样回给模型（**不重发它上一轮的输出**——_retry_parse 只带
# system + user，重发的只有指令；模型靠理由自己定位该改哪一处），比空转重试有效。
# 拼在 user 消息**末尾**（离生成点最近，权重最高）；理由来自 selection 域的
# SelectionError 文案（中文，本身即可读）。第三段是**可操作的改法**（方向 ③）：
# 长度 ≤ 1 的 instances 已在解析层确定性降级（工单 11 方向 ②），剩下能被拒的形态
# 除「≥2 个实例」外还有幻觉 slug / 词表外硬件名——三类都能用「先删后换」一句话
# 覆盖，故不按理由串做分支（无字符串判据、无新常量），一次把「一句内部事实」
# 变成「怎么改」。
DOMAIN_FEEDBACK_SEGMENT = (
    "【上一轮输出被拒绝，请修正后重新输出 JSON】\n"
    "拒绝理由：{reason}\n"
    "怎么改：优先「删」而不是「换」——被点名的模块条目删掉系统不认的字段"
    "（例如 instances）后，该模块按默认单实例使用；确实需要多个同型器件时，"
    "改选清单里带「多实例」标注的模块；库外硬件名则降级为硬件词表内的类别名。\n"
