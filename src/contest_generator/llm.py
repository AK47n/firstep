...
SELECT_SYSTEM_PROMPT = (
    ...
    "题目中没有提到的细节就是没有限制：题面未提及的要求（如指示灯颜色亮度、"
    "提示方式等）一律视为无限制，按合理默认实现，不为此提问。题面证据不足且"
    "直接影响模块选择或方案核心结构的关键信息，才在 questions 数组向用户补问，"
    "不要瞎猜；"
...
CLARIFY_SYSTEM_PROMPT = (
    ...
    "确认某条信息题面确实没有给出，才可提问；题目中没有提到的细节就是没有限制"
    "（默认宽松）：题面未提及的要求（如指示灯颜色与亮度、蜂鸣器音调、提示时长"
    "与方式、传感器具体型号等）一律视为无限制，按合理默认实现，不为此提问。"
    "题面证据不足且直接影响模块选择或方案核心结构的关键信息，才向用户补问——"
    "有疑问时一次性把所有疑问全部列出（每条具体可答、最多 "
