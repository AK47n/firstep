...
# models +1（继电器）→ 默认词表完整 wire 实测 7374（> 7334 fit 上限
# 7500−166，尾部类别被截、方案名丢失——test_wordlist_segment 契约红证），
# 预算升 7600（fit 上限 7434 ≥ 7374 全量送达 + 60B 余量；词表段全量 7374
# 比旧截断形态 7334 多 40B——最坏形态总量 +40B，全文预算 60400→60300 保
# 2KB 边界余量（batch5/7/8/9/12 口径）。
# 2026-09-06（wiki-modules-batch13）：无线通信模块 +1 方案更新（LoRa 数传
# solution 补 as32=串口版 + models +1 AS32）、感知传感器 +1 方案（BMP180）
# + models +1 → 默认词表完整 wire 实测 7557（> 7434 fit 上限 7600−166，
# 尾部类别被截、方案名丢失——test_wordlist_segment 契约红证），预算升 7800
# （fit 上限 7634 ≥ 7557 全量送达 + 77B 余量；词表段全量 7557 比旧截断形态
# 7434 多 123B——最坏形态总量 +123B，全文预算 60300→60200 保 2KB 边界余量
# （batch5/7/8/9/12 口径）。
WORDLIST_PROMPT_BYTES = 7800
