...
DISTILL_SYSTEM_PROMPT = (
    ...
    "内容摘要与结构配置对比判定哪些文件应该进母版。判定唯一判据：读文件内容后"
    "判断它是否通用（不依赖具体赛题）、是否基础建设必需（平台基础设施 / 工程"
    "配置），不看重复次数与出现范围。动作词表：keep（保留）/ merge（整合：同一"
    "路径多份内容不同时，读多份后整合出通用版本，选一份只是特例，必须给出整合"
    "产物全文与整合说明）/ exclude（剔除，如赛题专用业务代码、构建产物）。"
    "只输出 JSON 对象。"
)
