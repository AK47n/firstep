    ...
    sections: [
        ...
        blocks: [
          {
            type: "ul",
            items: [
              "「做题」组（生成 / 赛题库 / 代码 / 设置）：你日常用的——生成页是主流程，赛题库存历年真题（点「取题面」一键填入），代码栏是 IDE 式代码编辑器（打开生成工程直接看 / 改代码，多标签 + Ctrl+S 保存，见「代码栏」一节），设置里配 key 与体检。",
              "「资料管理」组（模块库 / 参考文件库 / PDF 资料库 / 母版）：内容管理页，做题用不到——除非要补录资料，先别动，尤其别删除。",
              "「指南」组（新手指引 / 版本更新记录）：新手指引就是你现在看的这个页面；版本更新记录按版本归纳工具改进的用户可见要点（新增 / 改进 / 修复 / 性能）。",
            ],
          },
          {
            type: "note",
            label: "一句话",
            text: "做题只碰「做题」组；想了解工具在迭代什么，去看「指南」组里的版本更新记录。",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            ...
            rows: [
              ...
              ["8 main.c 骨架", "AI 初始化好所有模块的 main.c 草稿（草稿进编辑框，可直接改再生成；磁盘版改过可点「加载为编辑内容」取回）", "要 AI"],
      ...
      },
      {
        ...
        blocks: [
          {
            type: "p",
            text: "工程生成后，你写逻辑的主路径不是打开 IDE 手抄代码，而是回到生成页第 11 步——里面有几个页签：任务推进（主路径）、修订（补赛题答疑）、参数速调（改参数）、交付（打包）。也想直接看 / 改工程代码？顶部「代码」栏就是 IDE 式编辑器（见下一节）。",
          },
          {
            type: "ul",
            items: [
      ...
      {
        title: "代码栏：IDE 式代码编辑器（不离开网页改工程代码）",
        blocks: [
          {
            type: "p",
            text: "顶部导航「代码」栏是内置的 IDE 式代码编辑器：左边文件树、中间多标签编辑区、右边大纲与搜索（可收起贴边）。适合直接看 / 改工程里任何文件——不用开 Keil / CCS 也能快速改代码、看结构。",
          },
          {
            type: "ul",
            items: [
              "文件树：选工程目录（就是第 9 步的输出目录）后，树里列出全部文件（.c / .h / .md…）；双击打开，多个文件并行开成标签页，点标签切换，标签可关闭；改了没保存的标签会有「•」脏点，关闭时会先确认。",
              "编辑与保存：文本文件可直接改；按 Ctrl+S 或点「保存」按钮立即写回磁盘，脏点随之消失。保存后第 9 步结果区 / 生成页的工程状态会同步刷新。",
              "代码栏编译：底部状态栏「编译」按钮——先自动保存全部未保存文件，再编译当前工程（平台自动识别，不用手选 STM32 / MSPM0）。失败时下方错误列表自动展开，点一条直接定位到出错文件的那一行；需要 AI 自动修复时，点面板上的「去生成页一键编译修复」。状态栏同排还有「烧录到板子」——同样先自动保存全部未保存文件，再烧录当前工程（平台自动识别，不用手选平台），结果面板中文汇报，工具 / 产物缺失会弹指引卡（带「去设置页配置」）。",
              "只读保护：.md 默认是预览态（点「编辑源码」切到编辑）、GBK 等非 UTF-8 编码文件、超过 1MB 的文本文件——都不允许编辑（有标记，怕误存损坏原文件）。",
              "大纲与搜索：右侧「大纲」列文件里的函数 / 宏定义，点一条跳到源码对应行；「搜索」可按文件名或内容找代码；当前文件内查找按 Ctrl+F（命中列表点一条直接跳行）、替换按 Ctrl+H（「全部替换」改的是编辑缓冲，Ctrl+S 落盘）。侧栏右缘有竖排小按钮，点击向右贴边收起，不占视野。",
              "保存冲突：编辑期间文件被外面改过（比如 IDE / 别的工具），保存时会弹窗问你要覆盖、放弃编辑还是重新加载，不会悄悄覆盖。",
              "与第 8 步联动：第 8 步 main.c 编辑框和代码栏互通——第 8 步的「编辑 main.c」直接切到代码栏打开该 main.c（「查看工程」则打开整个目录）；代码栏保存 main.c 后，第 8 步会提示「磁盘 main.c 已更新」，可点「加载为编辑内容」把磁盘版取回编辑框。",
            ],
          },
          {
            type: "note",
            label: "什么时候用代码栏",
            text: "任务推进仍是主路径（按步实现 + 随时问 AI + 即时编译）；代码栏适合「看一眼 / 单独改某个文件」——比如想按自己的思路微调某段逻辑，或只想改参数值（当然「参数速调」页签也能干后者）。",
          },
          {
            type: "jump",
            label: "打开代码栏逛逛",
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: "p",
            text: "点「烧录到板子」按钮（结果区 / 任务卡 / 代码栏状态栏都有）——工具自动定位固件产物（STM32 → user/Objects/*.hex，MSPM0 → Debug/*.out）、自动探测本机烧录器并执行，中文回报结果。",
          },
          {
            type: "ul",
            items: [
    ...
    sections: [
      {
        ...
        blocks: [
          ...
          {
            type: "p",
            text: "结果区的草稿 chips 点击即可在「代码」栏直接打开对应文件（.md 默认预览态，点「编辑源码」切到编辑、可保存）。",
          },
          {
            type: "note",
            label: "草稿不等于终稿",
