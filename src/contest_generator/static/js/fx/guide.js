        ...
        blocks: [
          ...
          },
          {
            ...
            items: [
              ...
              "代码栏编译：底部状态栏「编译」按钮——先自动保存全部未保存文件，再编译当前工程（平台自动识别，不用手选 STM32 / MSPM0）。失败时下方错误列表自动展开，点一条直接定位到出错文件的那一行；需要 AI 自动修复时，点面板上的「去生成页一键编译修复」。",
              "只读保护：.md 默认是预览态（点「编辑源码」切到编辑）、GBK 等非 UTF-8 编码文件、超过 1MB 的文本文件——都不允许编辑（有标记，怕误存损坏原文件）。",
              "大纲与搜索：右侧「大纲」列文件里的函数 / 宏定义，点一条跳到源码对应行；「搜索」可按文件名或内容找代码。侧栏右缘有竖排小按钮，点击向右贴边收起，不占视野。",
