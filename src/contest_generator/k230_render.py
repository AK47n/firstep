...
K230 侧发送脚本与主控侧解析（library/modules/ball_detect/code/
ball_detect_stm32.c 的 parse_ball_line）吃同一份 CSV 帧契约：字段顺序 /
...
# 球检测帧字段顺序（与 ball_detect_stm32.c parse_ball_line 的 get_field 序
# 严格一致：1=cx 2=cy 3=confidence 4=x1 5=y1 6=x2 7=y2）
BALL_FRAME_FIELDS = ("cx", "cy", "confidence", "x1", "y1", "x2", "y2")
...
BALL_FRAME_FORMAT = BALL_FRAME_PREFIX + "," + ",".join(
    "{" + field + "}" for field in BALL_FRAME_FIELDS
)
...
def render_ball_frame(
    ...
    """球检测帧文本：字段按 BALL_FRAME_FIELDS 契约序落位。"""
    return BALL_FRAME_FORMAT.format(
...
# str.format——BALL_FRAME_FORMAT 本身含 {} 花括号（format 字段），format
# 替换会二次解释破坏花括号，replace 不解释。
_TEMPLATE_VARS: dict[str, str] = {
    "ball_frame_format": BALL_FRAME_FORMAT,
...
def render_python_artifact(template: str) -> str:
    """python_artifact 模板渲染：{{ball_frame_format}} / {{no_detect_frame}} /
