...
void MPU6050_Init()
{
	MPU6050_Write(PWR_MGMT_1, 0x80);   // 设备复位
	delay_ms(100);                       // 等待复位完成
	MPU6050_Write(PWR_MGMT_1, 0x00);   // 唤醒，使用内部8MHz振荡器
	delay_ms(50);                        // 等待振荡器稳定
	MPU6050_Write(SMPLRT_DIV, 0x27);	 // 设置采样频率为200HZ
	MPU6050_Write(CONFIG, 0x00);       // 失能DLPF
	MPU6050_Write(GYRO_CONFIG, 0x18);	 // 陀螺仪量程   ±2000dps
	MPU6050_Write(ACCEL_CONFIG, 0x00); // 加速度计量程 ±2g
	MPU6050_Write(INT_ENABLE, 0x01);   // 使能INT引脚输出中断
}
