...
int in_pharmacy         = 0;  // 已进入药房，停止一切动作
...
#define PHARMACY_CHECK_DELAY  50   // 50周期 × 10ms = 500ms
static int pharmacy_delay_cnt = 0;

// ===== 十字路口动作定义（前置，供K230模块和路由表共同使用）=====
...
#define K230_RECOG_WINDOW  100       // 第2路口识别窗口(100×10ms=1000ms)
...
#define MOTOR_A_SCALE     1.40f   // 左轮速度缩放（>1=补偿弱侧，<1=抑制强侧）
#define MOTOR_B_SCALE     0.70f   // 右轮速度缩放（>1=补偿弱侧，<1=抑制强侧）
...
// ===== 速度闭环参数 =====
#define BASE_SPEED        15.0f   // 基础速度（编码器计数/10ms），从50降到15防飞车
// 交叉口转弯参数
#define TURN_DUTY      6000  // 转弯时正转轮占空比（12%）
#define TURN_DUTY_REV  10000 // 转弯时反转轮占空比（20%）：补偿电机反转方向更大的摩擦，使两侧出力均衡
#define GO_STRAIGHT_MS   200  // 检测到交叉口后直行时间(ms)（常规路口用）
#define GO_STRAIGHT_MS_FAR_LEFT   200  // 远端左转直行时间(ms)，左轮偏弱可减小
#define GO_STRAIGHT_MS_FAR_RIGHT  200  // 远端右转直行时间(ms)，左轮偏弱可增大
#define GO_STRAIGHT_MS_K230_LEFT  200 // 第2路口K230左转直行时间(ms)
#define GO_STRAIGHT_MS_K230_RIGHT 200  // 第2路口K230右转直行时间(ms)
#define TURN_MAX_MS     3000  // 转弯安全超时(ms)，防止传感器故障导致无限旋转
#define COOLDOWN_MS     800  // 转弯完成后冷却时间(ms)，防止重复触发
#define TURN_TARGET_DEG 60.0f // 目标转角：停车后惯性过冲则略减(如87)，转不够则略加
...
#define DEBUG_STOP_AFTER_TURN  0
...
// 左掉头（单阶段：原地旋转PHARMACY_TURN_DEG=160°，药房已停车无需直行）
// 返回: 0=旋转中, 1=完成
...
// 右掉头（单阶段：原地旋转PHARMACY_TURN_DEG=160°，药房已停车无需直行）
...
// PB5微动开关消抖计数
#define PB5_LIFT_STABLE  5  // 连续5次HIGH = 50ms确认（5×10ms中断周期）
...
// 远端导航参数
#define FAR_Y_CENTER             110   // Y-center阈值：第2路口/小T用（实测校准值）
#define FAR_Y_CENTER_LARGE_T     110   // ★大T路口专用：值越大触发越早，独立于FAR_Y_CENTER
#define FAR_Y_TOLERANCE          100   // Y坐标中心容差（像素）
#define FAR_Y_STABLE_CNT    5        // Y-center稳定帧数（5×10ms=50ms）
#define FAR_SCAN_DEG        10.0f    // 左转扫描Q3角度（小角度专用，不用TURN_TARGET_DEG）
#define FAR_RECOG_WINDOW    100      // 识别窗口周期数（100×10ms=1000ms）
#define FAR_RECOG_WARMUP    5        // 识别窗口预热帧数
#define FAR_TURN_TARGET_DEG 70.0f    // 大T字路口转弯目标角度（独立于常规路口的70°）
// 其他转弯参数复用：GO_STRAIGHT_MS(200ms)、TURN_DUTY(6000)、TURN_MAX_MS(3000ms)、COOLDOWN_MS(800ms)
#define YCENTER_SUPPRESS_CYCLES  35   // ★转弯后进入巡线时Y-center抑制周期数（50×10ms=0.5s），改0即关闭
...
#define FAR_RET_TCROSS_STABLE_CNT  3  // 连续3次检测到T字路口(3×10ms=30ms)才确认
...
{
    int j;
    for (j = 0; j <= 8; j++) { far_cand_conf[j] = 0.0f; far_cand_cx[j] = -1; }
}
    ...
    case FAR_SMALL_T_STOPPED:
    {
        ...
        if (far_cnt <= 0)
        {
            ...
            if (dest_index == far_small_left)
                ...
                far_turn_dir = 1;  // 右转（含dest==right或兜底）
...
#define RAMP_CYCLES  50   // 50周期 × 10ms = 500ms 完成加速
...
#define GYRO_DAMP_GAIN   0   // 陀螺前馈增益：你实测0.03最优
#define LINE_DEADBAND    0.5f    // 死区阈值：|偏差|<0.5时输出0，中心不"狩猎"
#define LINE_PID_OUT_MAX 6.0f    // 轮速差上限
#define RETURN_ARRIVE_SPEED_RATIO  0.40f  // 返程到站前降速至40%，确保终点白线足够帧数触发停车
...
// ===== OLED调试显示：每200ms刷新（放在pid_control，转弯期间也能更新）=====
	...
	// ===== 药房送达流程：红灯常亮 → 等待微动开关抬起 → 160°掉头 → 返程 =====
	if (in_pharmacy)
	{
		...
		case PHARMACY_WAIT_LIFT:
			...
			// PB5微动开关消抖：HIGH=开关抬起（PB5与GND断开，上拉读取为1）
	...
	#define DEBUG_TURN_ON_ALL_WHITE  0
