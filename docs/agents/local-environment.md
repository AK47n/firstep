...
| 版本 | 线上状态 | 说明 |
|---|---|---|
| **未发版（工作树）** | ⚠️ **比线上多了 path-budget 那笔**：资料库两族 LCD 厂商例程已改名（最长包内路径 223 → 194），`full_pack` 新增路径长度上限 200 的拒绝发版护栏 | 详见第 6 节；下一个包才会带上（`firstep-pack\` 里那份 v1.2.0 基线清单**已过期**，下次发版必须重新生成，别拿它当基线） |
