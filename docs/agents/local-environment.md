...
## 0. 交接区：main 上有什么还没到用户手上（2026-09-16 更新）

**当前状态：有一件待发（用户侧的包是缺的）。** 2026-09-16 修的 mspm0 母版 `.settings/`
（CCS 工程编码钉 `encoding/<project>=UTF-8`）**只在本机工作树**——线上 v1.2.0 完整包里
**没有这两个文件**（2026-09-15 的「编译产物不入库」清理误删，见第 7 节）。
装着 v1.2.0 的用户生成 CCS 工程时拿不到编码钉，中文注释在默认 GBK 的工作区里有乱码风险。
**这一件必须随下一个版本发出去**，否则他们手里的包永远是缺的。
...
**下次发版可用的基线（都是 2026-09-14 重发的产物，直接当 `-Baseline`）**：

| 文件 | 用途 |
|---|---|
| `firstep-pack\firstep-update-v1.2.0.files.txt` | 下次小发版的 `-Baseline` |
| `firstep-pack\firstep-full-v1.2.0.manifest.json` | 下次完整包的 `-Baseline`（**2026-09-14 重打的那份**，不是 09-13 的旧版） |
| `firstep-pack\firstep-full-v1.2.0.zip`（770.0 MiB） | 想省流量复跑档③ e2e 时可直接复用 |
| `firstep-pack\release-notes-v1.2.0-rerun.md` | 本次 Release 说明原稿（已发到线上；下次照模板改） |
...
**⚠ 未了**：线上 v1.2.0 的完整包里**仍然缺这两个文件**——老用户的包要等下一个版本才补上，
见第 0 节。

### 7.2 免费的三百兆：`refs/remotes/origin/*` 那 6 条旧分支

**症状**：`.git` = 591 MB，而 `git count-objects` 显示无松散垃圾（2 个 pack，`prune-packable 0`）。

**根因**：`--all` 有 **5287** 个提交，`main` 只有 **2780**——差额来自 6 条远端跟踪分支
...
`git clone` 只复制 heads/tags，所以**克隆里一 repack 就掉到 292.3 MB**（探针实测）。

| 口径 | .git |
|---|---|
| 现状 | 591.1 MB |
| 只 repack、不删任何历史（要先把那 6 条 ref 摘掉） | **292.3 MB** |
| repack + 从全部历史删 `sources/materials/`（重写，耗时 973s） | **123.4 MB** |

**注意**：那 6 条分支**线上还在**（`git ls-remote --heads origin` 六条都在），所以摘引用
不等于线上消失；真要减负得先决定这些 PR 分支是否废弃。**决策未定**——数字已测出。
探针 `.scratch/materials-history-purge/probe-A.ps1` 可复跑（它只在 `%TEMP%` 的硬链接克隆里动手，
原库只读；它导出的 4 MB `probe-A-after-revlist.txt` 已删，别入库）。
