...
**当前状态：main 上有 16 笔提交没进任何发布包（自 `main@7784fe8d` = v1.2.1 重发资产的内容），
且本轮拍板「不发版」。**
...
| 项 | 值 |
|---|---|
| 未发布的提交 | 自 `7784fe8d`（= v1.2.1 重发资产的内容）之后共 **16 笔**（含 CHANGELOG 自动提交与上一轮 `update-restart-stale-service` 的账本/证据提交；本轮的 4 笔功能提交是 `1e884b4b` / `9a9d881a` / `1afdf63f` / `87aa691e` / `58763892` / `f74ff04a`） |
...
| 版本 | 线上状态 | 说明 |
|---|---|---|
| **v1.2.1（2026-09-16 首发；2026-09-19 重发一次）** | ✅ 线上：八件套齐全，`/releases/latest` 指向本版。**重发后**：完整包 `801,928,759` B（sha256 `6f6d29f59129d187…`）/ 小发版 `304,785,030` B（sha256 `9b98364d2f39b6f0…`） | 首发内容：补回 mspm0 母版 `.settings/`（CCS 编码钉）——**v1.2.0 的用户装的就是缺的**。**重发内容**：启动器两处修复（旧进程判据 + 起服务前建数据目录，工单 `update-restart-stale-service/01-03`）+ 提交期闸门工具进包（清单 +10 条）。版本号**没动**、tag 仍指 `3263fa79`，资产内容 = `main@7784fe8d`（与 v1.2.0 两次重发同例） |
