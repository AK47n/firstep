"""mq5 液化气/天然气传感器模块：真实库 + 真实母版不变量与双平台单选生成。

与 mq135 同款结构测试：manifest 形状（双平台、依赖 adc、mspm0 单角色
MQ5_AO_CH5 = adc PB24——ADC12_0 sequence 八通道 MEM5 独立通道，与 mq2
的 MEM0 薄封装不同：多路气体同选时物理通道独立、无共读冲突；stm32 单角色
MQ5_AO = adc PA5——ADC 共享组共读（页面原脚即共读点））、mspm0 单选
...
def test_mq5_mspm0_master_syscfg_shared_facts():
    """母版 ADC12_0 八通道共享事实（endAdd 6→7、adcMem5chansel=CHAN_5、
...
def test_mq5_mspm0_single_select_generation(tmp_path):
    """mq5 mspm0 单选生成：syscfg 保留 ADC12_0（八通道 + mq5 通道行）、
...
# ---------------------------------------------------------------------------
# wiki-stm32-batch5/03：stm32 平台条目（页面 ADC 序列收敛 ml_adc）
# ---------------------------------------------------------------------------
...
def test_mq5_manifest_shape_stm32():
    """stm32 条目：单角色 MQ5_AO = adc PA5（macros = MQ5_AO_CH，ADC 共享组
    ...
    assert stm32.kit and stm32.source_url == (
        ...
        "module/sensor/mq-5-sensor.html"
    )
    for needle in (
        "lckfb-地阔星移植手册/sensor--mq-5-sensor.md",
        "SAMPLES 30",
        "酒精值",
        "未上板",
        "外部分路器",
...
def test_mq5_stm32_macro_defined_in_pin_config():
    """stm32 接线单源：MQ5_AO_CH 必须在母版 pin_config.h（ADC_Channel_5）。"""
    text = (STM32_MASTER / "pin_config.h").read_text(encoding="utf-8")
    assert re.search(r"#define\s+MQ5_AO_CH\s+ADC_Channel_5", text)


def test_mq5_stm32_single_select_generation(tmp_path):
    """mq5 stm32 单选生成：依赖 adc 展开、静态门禁通过、模块文件落盘、
    uvprojx 注册 mq5_stm32.c。"""
    resolved = resolve_selection(MODULES, PLATFORM_STM32, ["mq5"])
    assert {m.slug for m in resolved.manifests} == {"mq5", "adc"}
    out = tmp_path / "out"
    generate(
    ...
    assert (out / "modules/mq5/code/mq5_stm32.c").is_file()
    assert (out / "modules/mq5/code/mq5_stm32.h").is_file()
    uvprojx = next(out.rglob("*.uvprojx"))
    root = ET.parse(uvprojx).getroot()
    groups = root.findall("Targets/Target/Groups/Group")
    modules = next(g for g in groups if g.findtext("GroupName") == "modules")
    paths = [f.findtext("FilePath") for f in modules.findall("Files/File")]
    assert any("mq5_stm32.c" in p for p in paths)
    assert (out / "pin_config.h").is_file()


def test_mq5_stm32_code_guards():
    """stm32 代码层守卫：注释里有页面缺陷清单（SAMPLES 30/酒精值串台记录），
    剥离注释后零标准库/寄存器/演示残留/delay_1ms/IRQHandler；只吃母版
    ml_adc API（adc_init/adc_get + MQ5_AO_CH 宏）；源码零「酒精值」字面量。"""
    c = (MODULES / "mq5" / "code" / "mq5_stm32.c").read_text(encoding="utf-8")
    h = (MODULES / "mq5" / "code" / "mq5_stm32.h").read_text(encoding="utf-8")
    full = c + "\n" + h
    ...
    assert re.search(r"#define\s+MQ5_ADC_MAX\s+4095u", h)
    assert re.search(r"#define\s+MQ5_ADC_SAMPLES\s+5u", h)
    # 正向映射原式（页面 Get_MQ5_Percentage_value）
    assert "* 100.0f" in code_only and "/ (float)MQ5_ADC_MAX" in code_only
    assert "1.0f - " not in code_only
    # 只吃母版 ml_adc API + 引脚宏（无 ADC_Channel_5 字面量）
    assert "adc_init(ADC_1, MQ5_AO_CH)" in code_only
    assert "adc_get(ADC_1, MQ5_AO_CH)" in code_only
