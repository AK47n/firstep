"""mq2 烟雾/可燃气体模块：真实库 + 真实母版不变量与双平台单选生成。

与 us016 同款结构测试：manifest 形状（双平台、依赖 adc、mspm0 单角色
MQ2_AO_CH0 = adc PA24——与 adc/us016 共享 ADC12_0 MEM0 槽位薄封装，无新
ADC 通道；stm32 单角色 MQ2_AO = adc PA5——**ADC 共享组共读**（页面原脚
即共读点））、mspm0 单选生成（syscfg 裁剪保留 ADC12_0、mq2 + 依赖 adc 模块
文件落盘、main.c 调 init/read_percent 过静态门禁）与 stm32 单选生成（依赖
adc 展开 + uvprojx 注册）。百分比公式与「无 ADC 中断」源码守卫钉死（页面
...
def test_mq2_manifest_shape_mspm0():
    """mq2：双平台条目；依赖 adc；mspm0 单角色 = adc PA24（MEM0 槽位）。"""
...
# ---------------------------------------------------------------------------
# wiki-stm32-batch5/01：stm32 平台条目（页面 ADC 序列收敛 ml_adc）
# ---------------------------------------------------------------------------
...
def test_mq2_manifest_shape_stm32():
    """stm32 条目：单角色 MQ2_AO = adc PA5（macros = MQ2_AO_CH，ADC 共享组
    ...
    assert stm32.kit and stm32.source_url == (
        ...
        "module/sensor/mq-2-sensor.html"
    )
    for needle in (
        "lckfb-地阔星移植手册/sensor--mq-2-sensor.md",
        "SAMPLES 30",
        "PA27",
        "未上板",
        "外部分路器",
...
def test_mq2_stm32_macro_defined_in_pin_config():
    """stm32 接线单源：MQ2_AO_CH 必须在母版 pin_config.h（ADC_Channel_5）。"""
    text = (STM32_MASTER / "pin_config.h").read_text(encoding="utf-8")
    assert re.search(r"#define\s+MQ2_AO_CH\s+ADC_Channel_5", text)


def test_mq2_stm32_single_select_generation(tmp_path):
    """mq2 stm32 单选生成：依赖 adc 展开（adc 模块 stm32 条目 files=[] 内嵌
    母版）、静态门禁通过、模块文件落盘、uvprojx 注册 mq2_stm32.c。"""
    resolved = resolve_selection(MODULES, PLATFORM_STM32, ["mq2"])
    assert {m.slug for m in resolved.manifests} == {"mq2", "adc"}
    out = tmp_path / "out"
    generate(
    ...
    assert (out / "modules/mq2/code/mq2_stm32.c").is_file()
    assert (out / "modules/mq2/code/mq2_stm32.h").is_file()
    uvprojx = next(out.rglob("*.uvprojx"))
    root = ET.parse(uvprojx).getroot()
    groups = root.findall("Targets/Target/Groups/Group")
    modules = next(g for g in groups if g.findtext("GroupName") == "modules")
    paths = [f.findtext("FilePath") for f in modules.findall("Files/File")]
    assert any("mq2_stm32.c" in p for p in paths)
    assert (out / "pin_config.h").is_file()


def test_mq2_stm32_code_guards():
    """stm32 代码层守卫：注释里有页面缺陷清单（SAMPLES 30/PA27 串台记录），
    剥离注释后零标准库/寄存器/演示残留/delay_1ms/IRQHandler；只吃母版
    ml_adc API（adc_init/adc_get + MQ2_AO_CH 宏）。"""
    c = (MODULES / "mq2" / "code" / "mq2_stm32.c").read_text(encoding="utf-8")
    h = (MODULES / "mq2" / "code" / "mq2_stm32.h").read_text(encoding="utf-8")
    full = c + "\n" + h
    ...
    assert re.search(r"#define\s+MQ2_ADC_MAX\s+4095u", h)
    assert re.search(r"#define\s+MQ2_ADC_SAMPLES\s+5u", h)
    # 正向映射原式（页面 Get_MQ2_Percentage_value）
    assert "* 100.0f" in code_only and "/ (float)MQ2_ADC_MAX" in code_only
    assert "1.0f - " not in code_only
    # 只吃母版 ml_adc API + 引脚宏（无 ADC_Channel_5 字面量）
    assert "adc_init(ADC_1, MQ2_AO_CH)" in code_only
    assert "adc_get(ADC_1, MQ2_AO_CH)" in code_only
