...
def test_sht20_stm32_scl_init_guard():
    """SCL 初始化防回潮（批次 3/01）：init 必须含 gpio_init(SHT20_SCL_GPIO,
    SHT20_SCL_PIN, OUT_OD) + 置高——F1 复位后浮空输入、ODR 写入无效，
    不初始化 = 总线死（批次 2 六件 SCL 从未初始化的真 bug 回修）。"""
    c = (MODULES / "sht20" / "code" / "sht20_stm32.c").read_text(encoding="utf-8")
    code_only = strip_comments(c, keep_preprocessor=True)
    assert re.search(
        r"gpio_init\(SHT20_SCL_GPIO, SHT20_SCL_PIN, OUT_OD\)", code_only
    )
    assert "SHT20_SCL(1)" in code_only
