...
def test_cli_fix_max_rounds_matches_frontend() -> None:
    """CLI FIX_MAX_ROUNDS == 前端 ui/fix-center-core.js FIX_MAX_ROUNDS（改动须两处同步）。"""
...
def _fix_src() -> str:
    ...
    工单 code-ide-ai/05 流程抽 ui/fix-center-core.js——钉再指向核心模块）。"""
    return (
        REPO_ROOT
        / "src"
        / "contest_generator"
...
def test_index_html_round_cap_text_mentions_fix_continue() -> None:
    ...
    assert "可点「继续修复」再来" in _fix_src(), (
        "ui/fix-center-core.js 轮上限终态文案未指引「继续修复」按钮"
    )
...
def test_index_html_has_fix_continue_resume_snapshot() -> None:
    ...
    assert re.search(
        ...
        _fix_src(),
    ), "ui/fix-center-core.js 找不到 fixLoop.resume 续跑态快照"
