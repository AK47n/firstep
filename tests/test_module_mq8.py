...
mq8 + 依赖 adc 模块文件落盘、main.c 调 init/read_percent 过静态门禁）与
stm32 单选生成（wiki-stm32-batch6/05——页面 ADC 序列收敛 ml_adc、ADC 共享组
PA5 共读）。
...
# ---------------------------------------------------------------------------
# wiki-stm32-batch6/05：stm32 平台条目（页面 ADC 序列收敛 ml_adc）
# ---------------------------------------------------------------------------
...
def test_mq8_manifest_shape_stm32():
    """stm32 条目：单角色 MQ8_AO = adc PA5（macros = MQ8_AO_CH，ADC 共享组
    ...
    assert stm32.kit and stm32.source_url == (
        ...
        "module/sensor/mq-8-sensor.html"
    )
    for needle in (
        "lckfb-地阔星移植手册/sensor--mq-8-sensor.md",
        "SAMPLES 30",
        "酒精值",
        "未上板",
        "外部分路器",
...
def test_mq8_stm32_macro_defined_in_pin_config():
    """stm32 接线单源：MQ8_AO_CH 必须在母版 pin_config.h（ADC_Channel_5）。"""
    text = (STM32_MASTER / "pin_config.h").read_text(encoding="utf-8")
    assert re.search(r"#define\s+MQ8_AO_CH\s+ADC_Channel_5", text)


def test_mq8_stm32_single_select_generation(tmp_path):
    """mq8 stm32 单选生成：依赖 adc 展开、静态门禁通过、模块文件落盘、
    uvprojx 注册 mq8_stm32.c。"""
    resolved = resolve_selection(MODULES, PLATFORM_STM32, ["mq8"])
    assert {m.slug for m in resolved.manifests} == {"mq8", "adc"}
    out = tmp_path / "out"
    generate(
    ...
    assert (out / "modules/mq8/code/mq8_stm32.c").is_file()
    assert (out / "modules/mq8/code/mq8_stm32.h").is_file()
    uvprojx = next(out.rglob("*.uvprojx"))
    root = ET.parse(uvprojx).getroot()
    groups = root.findall("Targets/Target/Groups/Group")
    modules = next(g for g in groups if g.findtext("GroupName") == "modules")
    paths = [f.findtext("FilePath") for f in modules.findall("Files/File")]
    assert any("mq8_stm32.c" in p for p in paths)
    assert (out / "pin_config.h").is_file()


def test_mq8_stm32_code_guards():
    """stm32 代码层守卫：正向映射原式（页面 Get_MQ8_Percentage_value——
    无反向式）；剥离注释后零标准库/寄存器/演示残留/delay_1ms/IRQHandler/
    「酒精值」字面量（串台仅 notes 记录）；只吃母版 ml_adc API。"""
    c = (MODULES / "mq8" / "code" / "mq8_stm32.c").read_text(encoding="utf-8")
    h = (MODULES / "mq8" / "code" / "mq8_stm32.h").read_text(encoding="utf-8")
    full = c + "\n" + h
    ...
    assert re.search(r"#define\s+MQ8_ADC_MAX\s+4095u", h)
    assert re.search(r"#define\s+MQ8_ADC_SAMPLES\s+5u", h)
    assert "* 100.0f" in code_only and "/ (float)MQ8_ADC_MAX" in code_only
    assert "1.0f - " not in code_only
    assert "adc_init(ADC_1, MQ8_AO_CH)" in code_only
    assert "adc_get(ADC_1, MQ8_AO_CH)" in code_only
