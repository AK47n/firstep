...
def test_mq135_mspm0_master_syscfg_shared_facts():
    """母版 ADC12_0 五通道共享事实（endAdd 3→4、adcMem4chansel=CHAN_6、
    adcPin6=PB20 归 mq135——ir_distance/joystick 相关断言同步）。"""
...
def test_mq135_mspm0_single_select_generation(tmp_path):
    """mq135 mspm0 单选生成：syscfg 保留 ADC12_0（五通道 + mq135 通道行）、
...
def test_mq135_percent_formula_guard():
    ...
    assert "> ADC_Channel_5" in adc_source  # adc_get 守卫扩展至 MEM5
