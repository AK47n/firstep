...
test("推荐结果区与任务推进区边界说明", () => {
  ...
  assert.match(recommendFx, /代码 \/ 赛题逻辑疑问请到第 11 步「任务推进」问 AI/,
    "买件→任务推进边界");
});
...
test("generate-tasks 直达监听：切页签 + 展开全局商量 + 未加载目录提示", () => {
...
test("第 12 步交接提示词与收敛后入口呼应", () => {
  assert.match(html, /工具内已能「和 AI 商量」（第 11 步任务推进）/,
    "交接卡提示呼应入口收敛");
});
