...
def test_render_pin_config_uart_swap_irq_calls_regrouped():
    ...
    out = render_pin_config(STM32_MASTER_PIN_CONFIG, _resolve(SWAP_BINDINGS))
    assert (
        "#define USART1_IRQ_CALLS digit_uart_rx_handler(); coord_detect_rx_handler(); hc05_rx_handler(); fingerprint_rx_handler(); neo_6m_rx_handler();\r\n"
        in out
    )
    assert (
...
def test_render_pin_config_default_irq_calls_byte_identical():
    ...
    assert "#define USART1_IRQ_CALLS digit_uart_rx_handler(); coord_detect_rx_handler(); uwb_rx_handler(); hc05_rx_handler(); fingerprint_rx_handler(); neo_6m_rx_handler();\r\n" in out
