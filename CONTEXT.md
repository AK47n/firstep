...
| 简介 | manifest.description，判据四要素：① 与代码一致（AI 校验）；② 硬件身份可确认——套件型号（kit）与购买链接（source_url），平台条目字段、新录入必填、URL 格式校验、由人补填；③ 能力方向——声明可用于哪类赛题功能（如"灰度循迹""PID 闭环控制""K230 视觉帧解析"），允许多值点明主能力，补录必填；④ 无题绑定——禁题号/年份/具体题名/专用逻辑（"XX 题专用"类别已消灭，ADR 0009；机械拦截词表单源 = tests/test_module_universality.py） | library.py / llm.py（AI 校验）；结构测试机械拦截（黑名单词表单源） |
...
- 模块形态：模块 = 纯驱动切片（ADR 0009）——状态机/调度/流程逻辑归生成骨架，跨题复用逻辑归参考文件库；机械验证 = 结构测试黑名单（题号/年份/题名，词表单源，能力词白名单防误伤）+ AI 语义校验（判据③能力方向 / ④无题绑定）。
