...
| 巡线 | 灰度循迹类能力（21F / 2024H / 2026H 巡线题），模块库内按平台三层分置：灰度读取（stm32=pid 内 gray_track / mspm0=huidu）、循迹核心（stm32=pid.c / mspm0=motor 内嵌 adjust_motor_pwm 加权质心，原 huidu.c 并入）、题专用决策（路口检测 / 模式状态机 / 声光——stm32=pid 套装（gray_track 十字路口 + pid_isr 10ms 调度）；mspm0 两版：**2026H 滚球版已入库**（gray_track_mspm0 启停线检测 + LAP 状态机，98f8b0a）、**2024H 巡线题专用层缺**（白区计数路口 + AB/ABCDA/ACBDA 状态机 + 声光，源 = sources/car/car xunji 原生真机代码，已归档为 car-1-1 参考未补录，工单 mspm0-modules-backfill 01 剩余部分） | manifest 简介 / library/modules |
...
| 校验语料 | 生成前六道门禁共吃的内存语料：模块文件（文本 / 类别 / 所在目录）+ 母版头 + 母版搜索目录 + main.c，一次读盘；门禁退化为吃语料的纯谓词（可内存直构测试），不各自读盘；门禁装配唯一出处 = generator.GENERATION_GATES 表 + run_generation_gates（照 categories.RULE_CATEGORIES 先例）：顺序 / 输入依赖 / 全貌收进表，新增门禁 = 表加一条 + 谓词 | generator.py（ModuleCorpus / ModuleFile / build_module_corpus / GENERATION_GATES） |
...
| 收敛循环 | 推荐分析以题面为依据反复自检修订（删脑补 / 补遗漏 / 重查覆盖），连续两轮功能需求层一致即收敛（上限 4 轮）；拿不准暂停向用户补问（澄清阶段先行：补问在收敛前完成，回答后不重跑已跑轮次）；收敛解可缓存进赛题条目（v2，库内命中每次现算，库会变） | selection.py / webapp.py / llm.py |
