...
| 巡线 | 灰度循迹类能力（21F / 2024H 巡线题），模块库内按平台三层分置：灰度读取（stm32=pid 内 gray_track / mspm0=huidu）、循迹核心（stm32=pid.c / mspm0=motor 内嵌 adjust_motor_pwm 加权质心，原 huidu.c 并入）、题专用决策（路口检测 / 模式状态机 / 声光——stm32=pid 套装（gray_track 十字路口 + pid_isr 10ms 调度）/ **mspm0=car xunji 已归档未补录，缺口**，源 = sources/car/car xunji 原生真机代码，control.c 白区计数路口 + AB/ABCDA/ACBDA 状态机 + xunji_template 加权质心） | manifest 简介 / library/modules |
