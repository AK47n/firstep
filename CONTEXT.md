...
| 提炼 | 导入多旧工程 → 对比 → AI 判定 → 报告（保留 / 整合 / 剔除 + 残留清单）→ 确认 → 入库；判定唯一判据：读内容判断是否通用、是否基础建设必需，不看重复次数；确认是一条事务（confirm_distillation） | master.py + llm.py |
| 报告模型 | 提炼报告的判定条目与动作词表：形状 / 序列化 / "merge 必须带整合产物全文与说明"不变量唯一所有者；报告平台必须与工程平台一致（平台交叉校验） | report.py |
