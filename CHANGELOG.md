<!-- changelog-auto: last-commit=269620ecfd03a9cf813a1860e6bb731eb836b2f0 -->
# 更新记录
...
- 23:19 docs(沙箱演练): 第二梯队 B1–B5 真机演练收口——判据 35 条里 33 条成立，开出 4 张缺陷/决策单
- 23:21 docs(沙箱演练): 订正判据条数——35 条里 33 成立 / 2 不成立（上一提交信息写的 33/34 少算一条）

## 2026-09-16
- 19:16 docs(本机环境): 记账——清理线两笔账、测试跑法新口径、一处待发项
- 19:16 test(跑法): 全套默认超时（卡住 = 红）+ xdist opt-in——阈值 180s 有实测依据
- 19:16 fix(母版): 还原被误删的 mspm0 .settings/ 工程编码钉——清理器例外 + 闸门扩到工程配置 + 回归守卫
- 19:20 fix(更新记录): 历史重写后锚点失效——CHANGELOG 补录静默静止，现在会自愈并出声
- 19:31 v1.2.1 版本号三处同步 + 用户可见要点（mspm0 编码钉随包发出）
- 19:34 docs(本机环境): v1.2.1 已发（编码钉随包发出、缺口关闭）；仓库体积两笔账拍板——摘引用已做完省 299.2 MB、历史重写不做
...
- 20:57 清理：从全部历史里删掉编译产物（它们夹带本机路径与 Keil 许可证）

## 2026-09-14
- 09:20 fix(资料库): 两族 LCD 厂商例程路径减肥——包内最长路径 223 → 194 字符，Windows 资源管理器解压不再报「路径太长」（工单 path-budget/01）
- 09:26 fix(发版): 完整包加「包内路径 ≤ 200 字符」拒绝发版护栏 + 真树/资料库两条判据（工单 path-budget/01）
- 09:32 docs(下载): README 补「路径太长（0x80010135）」一节——解到盘根或用自带 tar，并写明点「跳过」会静默丢文件
- 09:50 chore(发版): 重发 v1.2.0——旧那版 Release 与 tag 删除后按修好的树重打两包重传；要点合并进 VERSIONS.md 同一个 v1.2.0 区块（不新开版本号）
- 12:41 fix(资料库): 完整包路径减肥 + 发版护栏——解压不再报「路径太长」
- 12:50 docs(本机环境): v1.2.0 重发记账——旧 release 已删、基线换新、已装用户触达缺口
- 19:47 fix(版本记录): 版本头格式修回契约 + 绑 __version__ 的守卫——记录页最新版不再漏掉当前版本
- 19:55 docs(本机环境): 记账——VERSIONS.md 版本头格式坑（静默跳过整块）与二次重传口径
- 19:55 docs(README): 当前版本行补记重发的两处修复
...
- 16:56 docs(下载): 两个业务流程长得像那 40 行另立工单 10——不与 09 合并，理由写在单里
...
- 23:06 docs(仓库): 重写 .gitignore 里被替换字符毁掉的 7 行注释——3 行还原自历史、4 行按上下文重写，忽略规则一行未改
- 23:13 chore(发版): v1.2.0 版本号三处同步 + 用户可见要点（下载抗断 / 新手装完就知道双击哪个）
- 23:15 docs(发版): 完整包体积口径跟到实测 770 MB（README 三处 + 发版流程四处）——库治理回收重复文件后包小了 50 MB，别再让新用户按旧数字估流量
- 23:22 docs(本机环境): v1.2.0 已发布——账目清零、真身仍是 v1.1.1 待一键更新、补发版实测口径与可复用基线
- 23:30 docs(本机环境): 真身已重启到 v1.2.0（没走下载）；补记「重启 ≠ 全量更新」——资料库基线只由完整包替换写回，现在不用管
- 23:33 feat(资料库): 本机基线可就地补——扫本地资料库并与完整包清单逐文件对比后落盘；顺带把一条把真机状态当夹具的用例改成注入
- 23:40 chore(资料库): 两份盘点器入库——体积构成（进包/不进包两条线）与被哪条排除规则挡掉的明细
