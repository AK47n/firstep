...
- [x] 执行 API：确认载荷（模块集 + 影响记录）→ 备份 → 覆盖式重生成 → diff 记录，SSE 进度事件
- [x] 备份：整树备份到输出目录外、带编号；回滚后目录内容与备份逐文件比对一致
- [x] 模块集变化 → 新骨架替换 main.c（旧版在备份）；模块集不变 → 跳过重生成、main.c 原样保留
- [x] 重生成后上下文清单同步更新（与执行时输入一致）
- [x] 失败路径：备份成功但重生成失败 → 目录保持可回滚状态，报错中文
