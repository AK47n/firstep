...
- [x] 分析 API：输入（上下文 + 新 Q&A）→ 输出（逐条影响结论 + 建议模块集 + 理由），SSE 进度事件（影响分析中 / diff 就绪）
- [x] 确定性 diff：新旧 slugs 集合差 → 增/删/不变清单（纯函数，可内存直构测试）
- [x] 平台警告按新模块集重算（复用既有警告逻辑）
- [x] 假 LLM 测试：结构化输出解析、非法输出重试兜底
- [x] 影响分析产物与 diff 可回传前端渲染（JSON 形状稳定）
