...
**Status:** resolved（2026-08-14 已合 main，主会话复核 + 1413 绿复跑；merge 记录见 Comments 尾注）
...
- 2026-08-14 主会话复核合并：diff 读码（boards.py 295 行 + 双板 JSON 能力集与 ml_libs 六表/地猛星 syscfg 互证 + manifest pins 模型 + 迁移面 spot-check gray_track/key.c/pin_config.h）；merge --no-ff 合入 main（主检出复跑 pytest 1413 绿 26.5s）；工单 02 接缝用词对齐实现（PinDeclaration.macros / pin_capability_instances）。
