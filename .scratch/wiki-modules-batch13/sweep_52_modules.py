# -*- coding: utf-8 -*-
"""批次 1-13 全部 52 件收官一致性快检（提交前只读扫描）。

镜像 .scratch/wiki-modules-batch12/sweep_48_modules.py 的检查面：
...
本批（收官小批）新增：relay（GPIO 迷你驱动）、as32（真实 UART 双向透传）、
bmp180 / ms5611（软 I2C 气压——海拔换算共用 44330 公式）。
"""
import json
import re
import sys
...
batch_slugs = [
    ...
    # 变体扩展既有 oled 条目）
    "lcd", "tp_xpt2046",
    # 批次 13（4——收官小批：relay 执行机构 / as32 无线数传 / bmp180+ms5611
    # 气压互替件）
