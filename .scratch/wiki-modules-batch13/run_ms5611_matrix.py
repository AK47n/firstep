# -*- coding: utf-8 -*-
"""ms5611 编译矩阵（批次 13/04）：单选生成 → SysConfig CLI（软 I2C GPIO 实例
合法性）→ gmake 真编译（0 error/0 warning 硬门槛——pow 链接 math，
ir_distance/bmp180 先例）。照 run_joystick_matrix.py 改 slug。"""
import sys
...
MAIN_C = (
    ...
    '#include "ms5611.h"\n'
    "int main(void) { ms5611_init(); float t = 0.0f, p = 0.0f; "
    "uint8_t ret = ms5611_read(&t, &p); (void)ret; "
    "float alt = ms5611_read_altitude(p); (void)alt; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch13" / "matrix" / "ms5611"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ms5611"])
generate(
