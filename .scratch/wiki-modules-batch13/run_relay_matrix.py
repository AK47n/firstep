# -*- coding: utf-8 -*-
"""relay 编译矩阵（批次 13/01）：单选生成 → SysConfig CLI → gmake 真编译
（0 error/0 warning 硬门槛）。照 run_joystick_matrix.py 改 slug。"""
import sys
...
MAIN_C = (
    ...
    '#include "relay.h"\n'
    "int main(void) { relay_init(); relay_set(1); relay_set(0); "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch13" / "matrix" / "relay"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["relay"])
generate(
