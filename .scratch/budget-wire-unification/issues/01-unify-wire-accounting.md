...
- [ ] 红证：只换结构测试口径 → select 最坏用例跑红（>128KB，记录字节数）
- [ ] 实施后：select/clarify 结构测试绿（json.dumps 口径，余量 ≥10KB）+ REFERENCE_FULLTEXT_BYTES 改大即红
- [ ] fix 侧结构测试逐字节不变绿（共享化零回归）
- [ ] 全量 pytest 绿（基线 1369）+ `mypy src` 干净
- [ ] `git status` 只出现预期文件
