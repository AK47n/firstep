# -*- coding: utf-8 -*-
"""relay stm32 编译矩阵（wiki-stm32-batch1/01）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0）。照 ir-beam-module/verify_compile.py
配方改 slug；产物在 .scratch/wiki-stm32-batch1/matrix/relay/。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { relay_init(); relay_set(1); relay_set(0); while (1); }\n"
)
...
out = REPO / ".scratch" / "wiki-stm32-batch1" / "matrix" / "relay"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["relay"])
generate(
