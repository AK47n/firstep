# -*- coding: utf-8 -*-
"""flame stm32 编译矩阵（wiki-stm32-batch1/02）：单选生成（依赖 adc 展开）→
UV4 真编译（0 error/0 module warning 硬门槛 = exit 0）。照 run_relay_matrix.py
改 slug；产物在 .scratch/wiki-stm32-batch1/matrix/flame/。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { flame_init(); (void)flame_read_percent(); while (1); }\n"
)
...
out = REPO / ".scratch" / "wiki-stm32-batch1" / "matrix" / "flame"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["flame"])
generate(
