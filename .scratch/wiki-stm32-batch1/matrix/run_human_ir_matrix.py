# -*- coding: utf-8 -*-
"""human_ir stm32 编译矩阵（wiki-stm32-batch1/03）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0）。照 run_relay_matrix.py 改 slug；
产物在 .scratch/wiki-stm32-batch1/matrix/human_ir/。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { human_ir_init(); (void)human_ir_read(); while (1); }\n"
)
...
out = REPO / ".scratch" / "wiki-stm32-batch1" / "matrix" / "human_ir"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["human_ir"])
generate(
