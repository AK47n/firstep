# -*- coding: utf-8 -*-
"""ws2812 stm32 编译矩阵（wiki-stm32-batch1/06）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0）。照 run_relay_matrix.py 改 slug；
产物在 .scratch/wiki-stm32-batch1/matrix/ws2812/。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { ws2812_init(); ws2812_set_led_count(8); "
    "ws2812_set_color(0, WS2812_RED); ws2812_refresh(); while (1); }\n"
)
...
out = REPO / ".scratch" / "wiki-stm32-batch1" / "matrix" / "ws2812"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["ws2812"])
generate(
