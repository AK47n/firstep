# -*- coding: utf-8 -*-
"""microwave_radar stm32 编译矩阵（wiki-stm32-batch1/04）：单选生成 → UV4
真编译（0 error/0 module warning 硬门槛 = exit 0）。照 run_relay_matrix.py
改 slug；产物在 .scratch/wiki-stm32-batch1/matrix/microwave_radar/。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { microwave_radar_init(); (void)microwave_radar_read(); "
    "while (1); }\n"
)
...
out = REPO / ".scratch" / "wiki-stm32-batch1" / "matrix" / "microwave_radar"
if out.exists():
    import shutil
    shutil.rmtree(out)
...
resolved = resolve_selection(
    REPO / "library" / "modules", PLATFORM_STM32, ["microwave_radar"]
)
generate(
