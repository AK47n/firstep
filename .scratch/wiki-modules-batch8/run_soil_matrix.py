# -*- coding: utf-8 -*-
"""soil 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "soil.h"\n'
    "int main(void) { soil_init(); float p = soil_read_percent(); (void)p; "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch8" / "matrix" / "soil"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["soil"])
generate(
