# -*- coding: utf-8 -*-
"""human_ir 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "human_ir.h"\n'
    "int main(void) { human_ir_init(); uint8_t d = human_ir_read(); (void)d; "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch8" / "matrix" / "human_ir"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["human_ir"])
generate(
