...
## 未采纳的方案：同根优先探测（**2026-09-18 用户拍板：维持现状**，属行为变更）
...
**2026-09-18 拍板结果（用户）**：**两条都维持现状**——① 同根优先**不做**（本机等于没变、
在有半个组件的机器上会挪动能编译的组合）；② 顶层独立安装（`mspm0_sdk_2_11_00_07` 这类 glob 外的
更「新」件）**也不认**，探测面不动。故本单「探测行为未改」的现状是**拍板后的既定行为**，
不是待办。全文见 `tracker-audit/2026-09-09-在途盘点.md` 末「待拍板项的拍板结果与留档」①②条。
