# -*- coding: utf-8 -*-
"""s12sd 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "s12sd.h"\n'
    "int main(void) { s12sd_init(); uint8_t i = s12sd_read_uv_index(); "
    "(void)i; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch9" / "matrix" / "s12sd"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["s12sd"])
generate(
