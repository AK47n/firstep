# -*- coding: utf-8 -*-
"""gp2y1014au 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { gp2y1014_init(); float d = gp2y1014_read_dust(); "
    "(void)d; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch9" / "matrix" / "gp2y1014au"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["gp2y1014au"])
generate(
