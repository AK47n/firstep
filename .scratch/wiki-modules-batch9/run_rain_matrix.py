# -*- coding: utf-8 -*-
"""rain 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "rain.h"\n'
    "int main(void) { rain_init(); float p = rain_read_percent(); "
    "(void)p; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch9" / "matrix" / "rain"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["rain"])
generate(
