...
> **⚠️ 第 2 节口径已被实施证伪（2026-09-17，见工单 08「立单方的账目更正」第 1 条）**：
> `format_wordlist_prompt` 的字节数**低估**真实代价（立单估 +1942 / 实测该口径 +416，
> 而**词表段实发 +770**）；真正的硬约束是**真实库最坏形态距自设 2KB 边界的余量**
> （基线只剩 **97B**），不是立单时写的「≤2500B」。**预算类结论请改用**
> `.scratch/recommend-domain-reject/measure-18-wordlist-budget.py`（两条传导路径的
> 段级实发增量）。本脚本第 1/3/4/5 节（覆盖率 / 现场名现算 / 对照 / 重复检查）仍然有效。
