...
- [x] 验收 1：create 文件——建成空文件（size 0）、父目录 mkdirs、已存在
  400 中文；返回 {path, size_bytes: 0, mtime_ns: 字符串}。
- [x] 验收 2：create 目录——makedirs(exist_ok=False)，已存在 400；返回
  {path}；嵌套路径（如 src/kernel/）一次建出。
- [x] 验收 3：rename——文件/目录均可改（os.rename 原子）；new_name 单段
  校验（空/含 `/ \ : * ? " < > |`/`.`/`..` → 400 中文）；源不存在 400；
  目标已存在 400；文件返回 {path, mtime_ns 字符串}，目录返回 {path}。
- [x] 验收 4：delete——文件删除；目录仅空可删（rmdir），非空 400 中文；
  不存在 400；返回 {removed: true}。
- [x] 验收 5：越界/非法路径（`..`、绝对路径、盘符、NUL 段）全部 400 中文
  （复用 _resolve_in_root 既有语义，无新增绕过）。
- [x] 验收 6：webapp 路由契约错误（缺 dir / 缺 path / type 非法 / new_name
