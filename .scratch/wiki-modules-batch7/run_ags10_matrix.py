# -*- coding: utf-8 -*-
"""ags10 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "ags10.h"\n'
    "int main(void) { ags10_init(); uint32_t voc = 0; "
    "uint8_t ret = ags10_read(&voc); (void)ret; (void)voc; "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch7" / "matrix" / "ags10"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ags10"])
generate(
