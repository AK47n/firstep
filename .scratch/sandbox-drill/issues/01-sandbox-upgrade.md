...
- [ ] 判据：替换后 `/api/health`.version == `1.2.1` —— **2026-09-18 不成立**：替换后服务仍报 **1.1.1**（旧进程没被停掉；启动器 `launcher.log` = `reason=already_running port=8020`）。**善后复测**：收掉旧进程重起 → **1.2.1**（用户自救口径成立）。根因 = 发起更新的那一代（v1.1.1）`spawn_updater` **没传 `--port`** → 更新器按 8000 停服（`updater.log`：`端口 8000 无监听进程，跳过停服`），详见缺陷单 `update-restart-stale-service/01`

  **2026-09-19 复跑：判据成立 ✓**（缺陷修复 + v1.2.1 资产重发之后，脚本一个字没改）：
  `served = 1.2.1`、`restarted_by_updater = true`，`launcher.log` 记
  `reason=started tries=1 port=8020 stale=1 served=1.1.1 disk=1.2.1`——走的正是新加的
  「踢掉旧进程再起」那条路；判红 0 / 卡住 0，四条隔离判据与十三条保命项全成立。
  起点是**还原出来的真 v1.1.1**（`.scratch/update-restart-stale-service/rebuild-sandbox-v111.py`）。
  证据：同目录 `verify-01-upgrade.{txt,json}`（本轮）与 `-b1-failed.{txt,json}`（09-18 那次）
