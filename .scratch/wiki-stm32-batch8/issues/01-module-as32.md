...
**实施清单：**
- [x] `library/modules/as32/code/as32_stm32.c/.h`（轮询收发：uart_pin_init_ex + uart_baud_config 9600 + 关 RXNEIE；send 走 uart_sendbyte 忙等；receive 轮询 SR RXNE → DR → 缓冲 cap 截断）
- [x] manifest.json platforms 增 stm32：files、dependencies []（mspm0 现状）、verified false、hardware_bound false、pins 2 行、kit/source_url（地阔星 wiki 原页）、notes（手册路径/原页/网盘 soft_asds.zip/采购/9600/UART3 仲裁/互替件同脚 + ZIGBEE 同实例默认共享先例/页面中断改轮询 + 截断修正/AT 范围外/未上板）
- [x] pin_config.h 增 AS32 宏段；母版 isr.c 无需改（轮询）
- [x] 测试 `tests/test_module_as32.py`（形状+宏存在+单选生成+mspm0 零改动+守卫：`AS32_RX_BUF_MAX 300u`、无 `IRQHandler`、无 `% AS32_RX_BUF_MAX`、9600、SR/DR 轮询）
- [x] test_pins.py 补 6 宏；test_default_layout.py PB10/PB11 登记 as32
- [x] UV4 矩阵 0/0（MAIN_C 调 init/send_string/send_hex/receive/flush (void) 化）→ verified=true
