...
**实施清单：**
- [ ] `library/modules/ir_remote/code/ir_remote_stm32.c/.h`（mspm0 实现换算：DL_GPIO_readPins → gpio_get、delay_us(20)；`_measure` 忙等 + 阈值 400-500/100-250/100-150/20-60/60-100/10-50 拍；反码校验）
- [ ] manifest.json platforms 增 stm32（files、dependencies ["delay"]、verified false、hardware_bound false、pins 1 行、kit/source_url 地阔星原页、notes：手册/原页/网盘/页面默认 PA2 弃用 + PA10 推理 + 与 TX PA9 错开/EXTI 改轮询/反码判断修正/20us 拍忙等不占 TIMER/未上板）
- [ ] pin_config.h 增 IR_REMOTE 宏段（2 宏）
- [ ] 测试 `tests/test_module_ir_remote.py`（形状+宏存在+单选生成+mspm0 零改动+守卫：`IR_TICK_US 20u`、`delay_us(20)`、`(uint8_t)~value` 反码校验、无 `EXTI`/`IRQHandler`/`TIM_`）
- [ ] test_pins.py 补 2 宏；test_default_layout.py PA10 登记
