# -*- coding: utf-8 -*-
"""as32 stm32 编译矩阵（wiki-stm32-batch8/01）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0）。产物在
.scratch/wiki-stm32-batch8/matrix/as32/。"""
import sys
...
out = REPO / ".scratch" / "wiki-stm32-batch8" / "matrix" / "as32"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["as32"])
generate(
