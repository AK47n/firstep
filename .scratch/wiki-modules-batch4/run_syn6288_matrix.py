# -*- coding: utf-8 -*-
"""syn6288 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "syn6288.h"\n'
    "int main(void) { syn6288_init(); syn6288_speak(\"lckfb\"); "
    "syn6288_stop(); syn6288_pause(); syn6288_resume(); "
    "syn6288_send_cmd(SYN6288_CMD_SPEECH, 0x00, \"ok\"); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch4" / "matrix" / "syn6288"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["syn6288"])
generate(
