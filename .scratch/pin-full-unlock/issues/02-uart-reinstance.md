...
```
实施 UART 换实例工单 .scratch/pin-full-unlock/issues/02-uart-reinstance.md：
1. 读工单 + .scratch/pin-full-unlock/spec.md（关键事实节必读）+ ADR 0012 + 最新 main（前置 01 已合）
2. pin_bindings.py：uart 类型级（TX/RX 对实例集交集非空，空 = 400 成对绑定）
3. generator.py：_check_uart_instance_conflicts 入 GENERATION_GATES（绑定实例 × 未绑定角色默认实例 400；
   绑定×绑定放行；默认×默认不查）
4. ml_uart.c：fputc 改 DEBUG_UART_INST 宏；新增 uart_pin_init_ex（引脚宏参数化，旧函数保留）
5. 五模块 init 改调 uart_pin_init_ex 传宏（核查 rx_handler 非 static）
6. pin_config.h：每角色 +4 引脚宏（原值）+ USARTx_IRQ_CALLS 三聚合宏；pinwriter 加
   _IRQ_CALLS/_TX_GPIO/_TX_Pin/_RX_GPIO/_RX_Pin 尾形（重分组 + 值推导同源）
7. 母版新增 isr.c：5 个 __weak rx_handler 兜底 + 3 个 USARTx_IRQHandler 调聚合宏
8. manifests：TX/RX 条目补 macros；index.html pinCanHost uart 类型级镜像
9. 红证先行 + 绿证（三组绑定全换位放行 + 宏值 + CALLS 重分组 + 默认逐字节）+ test_pins 同步
10. 真机：2026C 不配回归（== 新母版逐字节）/ 换位绑定 UV4 0 错 0 警 / 单角色撞车 HTTP 400
11. 提交 + 推送开 PR
注意：独立 worktree；文件边界见工单；printf 默认流已挪 USART2/PA2（真机验收注意）
```
