...
```
实施默认冲突重排工单 .scratch/pin-full-unlock/issues/05-default-conflicts.md：
1. 读工单 + .scratch/pin-full-unlock/spec.md（关键事实节必读）+ ADR 0012 + 最新 main（前置 01-04 已合）
2. 逐组裁决 5 组冲突：新默认脚按能力兼容 + 避开骨架 TIM2/3 + 不新撞约束选；不可达则最小化并 Comments 明示
3. manifests default 值 + pin_config.h 母版宏值 + macros 双向同步（每角色改三处）
4. tests/test_pins.py 宏值表逐项更新 + 新增 tests/test_default_layout.py
   （默认两两互异不变量 + 共享白名单）
5. 真机：2026C/2021F 不配回归 UV4 -r 0 错 0 警 + pin_config.h == 新母版逐字节
6. 验收记录写新旧默认对照表
7. 提交 + 推送开 PR
注意：独立 worktree；文件边界见工单；零 src/ 改动；默认形态变化 = 用户已验收形态变化，对照表必写
```
