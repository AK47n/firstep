...
- [x] 状态栏左侧保留目录路径（过长省略号收缩），右侧依次显示：`Ln X, Col Y`、语言徽标（C / XML / MD / TXT）、编码（UTF-8 / 非 UTF-8 标「非 UTF-8（只读）」）、缩进（空格: 4）、缩放百分比
- [x] Ln/Col 随光标移动（键盘 / 鼠标 / 跳行）实时刷新；无打开文件时显示占位（如「未打开文件」）
- [x] 缩放百分比随 Ctrl+滚轮缩放实时更新
- [x] 四个现有按钮（保存全部 / 编译 / 烧录 / 去生成页）保留、样式统一为状态栏条钮（不换行、flex:none 不挤压）
- [x] 窄视口（< 900px）信息段折叠为仅 Ln/Col + 缩放，不挤占按钮
- [x] 深浅主题下观感协调（只用既有 token，不新造色值）
- [x] tests/js 新增状态栏格式化纯件单测（0 行 / 只读 / 越界光标 / 中文路径等边界）
- [x] CDP 冒烟：打开文件 → 状态栏出现 Ln:Col；移动光标 → 数值变化
