# -*- coding: utf-8 -*-
"""l298n 编译矩阵：单选生成 → SysConfig CLI → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "l298n.h"\n'
    "int main(void) { l298n_init(); l298n_set_duty(1000); "
    "l298n_set_direction(1); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch10" / "matrix" / "l298n"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["l298n"])
generate(
