# -*- coding: utf-8 -*-
"""open_mv4 编译矩阵：单选生成 → SysConfig CLI → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "open_mv4.h"\n'
    "int main(void) { open_mv4_init(); int cx = 0, cy = 0; float conf = 0.0f; "
    "uint8_t ok = open_mv4_read_frame(&cx, &cy, &conf); (void)ok; (void)cx; "
    "(void)cy; (void)conf; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch10" / "matrix" / "open_mv4"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["open_mv4"])
generate(
