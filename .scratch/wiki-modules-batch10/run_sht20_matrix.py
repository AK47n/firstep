# -*- coding: utf-8 -*-
"""sht20 编译矩阵：单选生成 → SysConfig CLI → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "sht20.h"\n'
    "int main(void) { sht20_init(); float t = 0.0f, h = 0.0f; "
    "uint8_t ok = sht20_read(&t, &h); (void)ok; (void)t; (void)h; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch10" / "matrix" / "sht20"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["sht20"])
generate(
