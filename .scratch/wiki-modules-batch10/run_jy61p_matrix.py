# -*- coding: utf-8 -*-
"""jy61p 编译矩阵：单选生成 → SysConfig CLI → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "jy61p.h"\n'
    "int main(void) { jy61p_init(); float r = 0.0f, p = 0.0f, y = 0.0f; "
    "uint8_t ok = jy61p_read_angles(&r, &p, &y); (void)ok; (void)r; (void)p; "
    "(void)y; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch10" / "matrix" / "jy61p"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["jy61p"])
generate(
