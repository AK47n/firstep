...
**Status:** resolved（2026-08-14，实施记录见 Comments）
...
- [x] 红证：结构测试在现行 49152 cap 下跑红（> MAX_REQUEST_BYTES）
- [x] 修复后：结构测试绿（json.dumps 口径，余量 ≥10KB）+ cap 改大即红
...
### 2026-08-14 实施记录（本会话，Status → resolved）

**红证（先行）**：`tests/test_llm.py::test_fix_prompt_worst_case_fits_request_budget` 先写先跑——现行 49152 字符口径下断言 `total <= MAX_REQUEST_BYTES - 10*1024` 失败：**最坏形态 308085 字节 > 120832 预算（131072 − 10KB）**，超上限 2.35×（文件上下文全中文单段 wire ≈150KB + 回喂 60 条 ≈74KB 无界）。失败形态与工单预判一致：预检 LLMError 无 kind → 3 次快重试同尺寸必败 → 修复中心最后防线断。

**实施**（文件边界内 4 文件，208+/17− 行）：

- `fix_errors.py`：`FIX_CONTEXT_TOTAL_CHARS`(49152 字符) → **`FIX_CONTEXT_TOTAL_BYTES` = 23000（wire 字节记账）**。`_wire_size` = `len(json.dumps(s, ensure_ascii=True)) - 2`（与 `llm.py:1329` 预检同口径，中文 \uXXXX 6 字节/字符）；`_fit_wire_budget` 二分取预算内最长前缀（wire 随前缀单调不减；中文保留 ≈预算 1/6 字符，ASCII 近全额保留）；`read_file_contexts` 按 wire 字节扣预算 + 既有「上下文预算限制」标注。
- `llm.py`：**`FIX_PREVIOUS_FIXES_CAP` = 2500**——回喂段级合计截头带标注（`truncate_content` 单源，与 `CLARIFICATION_HISTORY_CAP` 同哲学），尾部条目裁掉（首条 = 最近一轮）；常量注释含完整预算反推算式（系统提示词 3.3KB + JSON 壳 + 报错 24.3KB + 赛题 24KB + main.c 24.3KB + 回喂 15.5KB + dropped/模块/标题 ≈5.3KB ≈ 96.4KB 基础段 → 余量给文件上下文）。
- 测试 +5（1361 = 基线 1356 + 5）：结构测试（json.dumps 口径，`read_file_contexts` 真截断路径端到端）、回喂段截断断言、wire 字节记账用例（红证：旧字符口径必红）、ASCII 近预算整段保留、`_fit_wire_budget` 精确/最长前缀直测。

**调参绿 + cap 改大即红**：结构测试最坏形态总量 **119532 字节，余量 11540（11.3KB）≥ 10KB**；`FIX_CONTEXT_TOTAL_BYTES` 23000→49152 即红（145684 字节）、`FIX_PREVIOUS_FIXES_CAP` 2500→5000 即红（+15KB 超余量）——两上限双双敏感，均已还原。

**口径决定（偏离工单预设一处，说明理由）**：结构测试赛题段取 4000 字符截断上限而非「2026C 实测 2626」——2021F 实测 2796 > 2626，实测形态不是最坏形态；结构保证须按 `_truncate_content` 上限取形（2626 与 2796 都在 4000 内，按上限取仍绿，代价 = 文件上下文预算少 ~8KB，换保证无洞）。dropped 清单按 200 条 × ~20 字符覆盖（工单定夺不强加上限，实测 KB 级）。

**既有全绿**：`pytest tests/` **1361 passed**（28.7s）+ `mypy src` 37 文件干净；`git status` 只出现预期 4 文件。

**（可选）真机 HTTP 项未跑**：HTTP 层预检逻辑（`llm.py:1329`）零改动，结构测试已按预检同口径钉死最坏总量 < 128KB（构造时走真实 `read_file_contexts` + `_fix_errors_user_prompt` + `json.dumps`）；留待下次真实修复流程自然覆盖（合成超大中文文件需真实 DeepSeek 修复调用，分钟级 + 成本）。

**会话教训**：sed -i 常量往返（23000→49152→23000）撞 Windows pycache mtime/大小同值 → 陈旧 pyc 致全量套件假红 2 条（145684 与 49152 形态吻合才定位）；清 `__pycache__` 复跑全绿。同长度常量往返编辑务必清缓存或改用不同长度验证值。
