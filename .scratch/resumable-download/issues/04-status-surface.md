...
> **后续（工单 05 的双轴评审）**：本单定下的 `message` = 「原因 + 第 N 次 + 从 X% 接着下」
> 这句话，在 05 被证明**不该由后端拼**——前端为了显示「正在自动重试（第 N 次）：…从 X% 接着下」
> 只能反过来解析它（两个轴都判了这条）。05 把三件事拆成三个字段：
> `message`（只说原因）/ `retry_count` / **`resume_percent`**（本单之外新增的第四个字段）。
> 矩阵在 `tests/test_download_status_surface.py` 里随之更新——**状态面的形状以那份测试为准**。
