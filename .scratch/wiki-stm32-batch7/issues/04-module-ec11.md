...
**code-review 整改（2026-09-07 收尾）：标准轴判断项 ②——`ec11_prev_a` 静态初值 =1 依赖上拉默认高（上电 A 相机械触点闭合段 = 低电平会误记 1 步虚假增量）——**已整改**：ec11_init 快照 A 相当前电平为轮询基准（跳过上电首拍）+ ec11_accum int16_t 溢出范围注释（±32767 ≈ 每格 2 边沿 1.6 万格——调用方节拍轮询不会堆积）；整改后测试全绿 + UV4 矩阵重跑 0 error/0 module warning（verified=true 维持有效）。**
