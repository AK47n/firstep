...
**Definition of done:**
- [ ] index.html 视觉段加下拉 + 提示区（id=`set-vision-provider` / `vision-provider-hint`），三输入框字段 id 零改动
- [ ] 回显后按 base_url 推断模式（bigmodel.cn → zhipu；api.deepseek.com 或空 → deepseek；其它 → custom），不覆盖用户字段
- [ ] 切换：deepseek → 填预设 + key 清空；zhipu → 填预设 + 掩码回填；custom → 字段不动；placeholder/hint 随模式更新
- [ ] 手动改 base_url/模型与预设不符 → 自动转 custom（`visionApplyingPreset` 防回调；key 手改不触发）
- [ ] PUT 组装零改动（仍读三输入框，掩码沿用走既有 `_masked_optional_key`）
- [ ] 全量 pytest 回归（后端未动，应 2123 绿）；手工路径清单写 Comments；Status 改 resolved
