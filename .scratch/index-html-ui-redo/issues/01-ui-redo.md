...
- [ ] 浏览器人工验收：美化项逐项过 + **引脚配置卡回归**（选平台出板图、旋转按钮、点引脚绑角色、板载共用 ⚠ 警示、改绑定生成 UV4 0 错）——待用户浏览器验收
- [x] 独立 worktree + 提交（`style:` 前缀）+ PR（待开）
...
- **用户勾选范围**（开工前确认）：仅 **板图卡 7 说明与图例**——文字排版、
  图例、旋转按钮排版、固定资源/板载共用警示区块标题。
- **改动**（仅 index.html，纯样式/排版层）：
  ...
  - `#pin-board-caption` 加粗（板名在 SVG 外永远正对，观感更清晰）。
