# -*- coding: utf-8 -*-
"""mq8 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "mq8.h"\n'
    "int main(void) { mq8_init(); float p = mq8_read_percent(); (void)p; "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch11" / "matrix" / "mq8"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["mq8"])
generate(
