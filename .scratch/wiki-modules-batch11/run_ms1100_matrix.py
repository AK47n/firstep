# -*- coding: utf-8 -*-
"""ms1100 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "ms1100.h"\n'
    "int main(void) { ms1100_init(); float p = ms1100_read_percent(); (void)p; "
    "while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch11" / "matrix" / "ms1100"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ms1100"])
generate(
