...
9. 作为学生，选旧式单实例 led 时，生成产物和以前逐字节一样，不破我已有的工程习惯。
...
- 新文件 `led_instances.h`（通道宏 `LED_RED` … + 每实例 pin 宏），**不碰母版
  `ml_led.h` / `pin_config.h`**——单实例路径母版零 diff（D4）；
...
旧单选 = 1 个默认实例（led 单实例仍生成红/黄/绿三通道，行为逐字节不变）。
`pinwriter` 的「不变不写」契约（文本无变化不落盘）是逐字节护栏。
