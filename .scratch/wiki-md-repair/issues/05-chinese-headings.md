...
- [ ] `parse_main` 返回 (md, img_urls, page_title)：捕获 `<h1>` 文本（剥锚点/`\u200b`；h1 仍在正文外）。
- [ ] `build_markdown`：顶部 `# 中文标题`（无 h1 回退 slug）；`- 标题：` 保留完整 `<title>`；
      `- 分类：{cat_label(cat)}`；`cat_label` 映射 sensor/screen/rf/control → 传感器类/显示类/无线通信类/控制类（未知回退原码）。
- [ ] `fetch_wiki.py`：parse_page 用 h1 或 `<title>` 去后缀作标题；模块索引 `## ` 用 cat_label。
- [ ] `tests/test_wiki_md.py` 覆盖：h1 捕获（含锚点/`\u200b`）、build_markdown 中文标题 + 分类映射、无 h1 回退 slug。
- [ ] 全量重抓 70 篇（0 失败）；质量扫描 70/70（新增：每篇 H1 含中文）；人工抽查 3 篇。
