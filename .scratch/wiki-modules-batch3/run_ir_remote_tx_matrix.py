# -*- coding: utf-8 -*-
"""ir_remote_tx 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { ir_tx_init(); ir_tx_send(0xE0, 0xFD); "
    "ir_tx_send(0x01, 0x45); ir_tx_send_repeat(); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch3" / "matrix" / "ir_remote_tx"
if out.exists():
    import shutil
    shutil.rmtree(out)
