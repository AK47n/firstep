...
- [x] 验收 1：preview 不落盘（mtime 不变、返回 new_content 与手验一致）。
- [ ] 验收 2：写模式成功 → 磁盘内容 = 应用后内容，返回新 mtime_ns。
- [ ] 验收 3：base_mtime_ns 不匹配 → 409（与 save 同口径消息模式）。
- [x] 验收 4：kind 非法 / path 越界 / 锚点不匹配 → 400 中文；`hunk.line` 越界或不对齐**不报 400**（有意：应用以 old 段整行匹配为锚，line 仅作展示——原「行号越界/不对齐 → 400」口径已修订）。
- [x] 验收 5：pytest 新增用例（test_codeview.py 或新文件）全绿 + 全量回归绿。
...
- `hunk.line` 越界/不对齐不报 400 属实现期有意偏离（应用以 old 段整行匹配为锚，line 仅展示）；kind / path / 锚点三类仍 400。
