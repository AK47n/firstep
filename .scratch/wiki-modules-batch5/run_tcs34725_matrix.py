# -*- coding: utf-8 -*-
"""tcs34725 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "tcs34725.h"\n'
    "int main(void) { tcs34725_init(); tcs34725_set_gain(TCS34725_GAIN_4X); "
    "tcs34725_set_integration_time(TCS34725_INTEGRATIONTIME_50MS); "
    "TCS34725_RGBC rgb = {0}; TCS34725_HSL hsl = {0}; "
    "if (tcs34725_read_rgb(&rgb)) { tcs34725_rgb_to_hsl(&rgb, &hsl); } "
    "tcs34725_disable(); tcs34725_enable(); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch5" / "matrix" / "tcs34725"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["tcs34725"])
generate(
