# -*- coding: utf-8 -*-
"""mlx90614 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "mlx90614.h"\n'
    "int main(void) { mlx90614_init(); float obj = 0.0f; float amb = 0.0f; "
    "(void)mlx90614_read_object_temp(&obj); "
    "(void)mlx90614_read_ambient_temp(&amb); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch5" / "matrix" / "mlx90614"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["mlx90614"])
generate(
