...
| 候选 | 内容 | 需要什么才动 |
|---|---|---|
| A. 门禁容量诊断 | 门禁命中时把「改绑或去掉冲突模块」升级为可操作数字：「落点 42 > 可用 31 脚、空闲脚已用尽，至少要去掉/替换 N 个模块」；判据复用现成两件（solver 剩余组 + 落点数 vs 可用 IO），前端零改动 | 真机里再遇到一次「用户照提示去改绑但根本无脚可改」的现场，或直接拍板做（成本小） |
| B. 口径复议 | ① stm32 默认撞脚是否从「提示」升级为「拦」（ADR 0010 复议：编译能过但接线错）；② 推荐/选择侧是否把容量当约束（像互斥组那样，别一次选满 15 个模块） | 需用户拍板口径（B 牵动 ADR 与推荐链路，工作量数倍于 A） |
