...
| 候选 | 内容 | 需要什么才动 |
|---|---|---|
| A. 门禁容量诊断 | 门禁命中时把「改绑或去掉冲突模块」升级为可操作数字：「落点 42 > 可用 31 脚、空闲脚已用尽，至少要去掉/替换 N 个模块」；判据复用现成两件（solver 剩余组 + 落点数 vs 可用 IO），前端零改动 | **✅ 已拍板放行（2026-09-18）；本单已立**（`.scratch/pin-capacity/spec.md` + `issues/01-02`，只升文案不增拦截，待做） |
