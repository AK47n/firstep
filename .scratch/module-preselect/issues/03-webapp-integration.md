...
- [x] `library_fingerprint` 用**全量摘要**（`topic.library_summaries`）——原口径「改用预筛后摘要列表」已由后续工单 preselect-recall-visibility/01 刻意反转（缓存语义 = 同题面同库同指纹；取全量防子集外模块陈旧推荐）。
...
- ① 本单验收清单原有两份逐字重复的副本（一份已勾、一份未勾），已删除未勾副本；② 「指纹改用预筛后摘要」被后续工单 preselect-recall-visibility/01 刻意反转为全量摘要。
