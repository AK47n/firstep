...
- [x] 过滤输入逐键过滤函数/宏/include 混合列表；无匹配显示空态；清空恢复全列表。
- [x] Enter 跳转当前选中符号行；点击条目跳转行。
- [x] Ctrl+Shift+O 展开 + 聚焦过滤框；再次按下不与框内输入冲突。
- [x] node 单测 symbolFilter（子串/前缀/大小写/空 query/无匹配）；CDP 冒烟 smoke-03。
