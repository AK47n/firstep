...
- [ ] Ctrl+W 关干净标签；关脏标签弹确认（与中键关闭一致）；关最后一个标签行为一致。
- [ ] Ctrl+B 开合侧栏，与点按钮行为一致；编辑区 textarea 聚焦也可触发；焦点在侧栏/面板输入框（查找/过滤）内 Ctrl+B 不抢键。
- [ ] 帮助窗口出现 Ctrl+W / Ctrl+B 中文条目。
- [ ] CDP 冒烟 smoke-02 真机验证两键可拦截；若被浏览器吞键 → 按退路换键并同步帮助数据与冒烟断言。
