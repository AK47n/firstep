// 复现 4：同目录 big.c 深滚 → 打开 small.c(30 行) → 检查窗口/水印/可点击性
...
const DIR = "C:\\Users\\luoji\\Desktop\\firstep\\.scratch\\code-editor-refine\\sample-proj-11";
await Eval(`import('/js/ui/codeview.js').then((m) => m.openCodeViewer('${DIR.replace(/\\/g, "\\\\")}'))`);
await waitFor(`!!document.querySelector('#code-tree [data-code-file="small.c"]')`);
await Eval(`document.querySelector('#code-tree [data-code-file="big.c"]').click(); true`);
await waitFor(`document.querySelector('#code-tabs .code-tab.on')?.dataset.tabPath === 'big.c'`);
...
await Eval(`document.querySelector('#code-tree [data-code-file="small.c"]').click(); true`);
await waitFor(`document.querySelector('#code-tabs .code-tab.on')?.dataset.tabPath === 'small.c'`);
...
const d = await Eval(`(() => {
  ...
  return {
    ...
    editH: document.querySelector('#code-viewer .code-edit')?.style.height,
    lineHits,
  };
})()`);
console.log(JSON.stringify(d, null, 2));
const shot = await cdp("Page.captureScreenshot", { format: "png" });
writeFileSync(join(ROOT, ".scratch", "code-editor-refine", "diag-switch2.png"), Buffer.from(shot.result.data, "base64"));
process.exit(0);
