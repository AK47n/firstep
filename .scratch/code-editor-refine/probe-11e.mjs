...
console.log(JSON.stringify(await Eval(`(() => {
  ...
  ta.value = b;
  ta.dispatchEvent(new Event('input', { bubbles: true }));
  const d = window.__syncDbg || {};
  return { totalMs: total.toFixed(1), t0to1: (d.t1 - d.t0).toFixed(1), t1to2: (d.t2 - d.t1).toFixed(1), t2to3: (d.t3 - d.t2).toFixed(1), totalDbg: (d.t3 - d.t0).toFixed(1) };
})()`), null, 2));
process.exit(0);
