# -*- coding: utf-8 -*-
"""l298n stm32 编译矩阵（wiki-stm32-batch9/04）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0；确认 TIM3_CH1/CH2 双通道编译）。
产物在 .scratch/wiki-stm32-batch9/matrix/l298n/。"""
import sys
...
out = REPO / ".scratch" / "wiki-stm32-batch9" / "matrix" / "l298n"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["l298n"])
generate(
