...
### 各件决策（2026-09 回填，页内事实已取证——batch2-facts.md）

| 工单 | slug | 手册 | 器件地址 | 母版 pin_config.h 宏段 | API（与 mspm0 对齐） | 关键决策 |
|---|---|---|---|---|---|---|
| 01 | aht10 | sensor--aht10-temp-humi-sensor.md | 0x38（写 0x70/读 0x71） | `AHT10_SCL_GPIO/_SCL_PIN/_SDA_GPIO/_SDA_PIN` | `aht10_init`（50ms 稳定+软复位 0xE1 0x08 0x00 校准）+ `aht10_read(t,h)`（0xAC 0x33 0x00 → 20ms+5×1ms 读地址重试 → 6B：湿度 20bit = b[1..3] 高 4 位、温度 = b[3] 低 4 位+b[4..5]；湿度 = d/2^20×100、温度 = d/2^20×200−50；0=成功/1=超时）+ `aht10_read_temperature/read_humidity` | mspm0 版：AHT10Reset+Init 合并（页面每次读后重复复位初始化——已修正）；地址 0x38<<1 原式保留；页面缺陷 pages（待 facts 回填）；页面默认脚 PA/（截断见 issue）**不采用** → 默认 PA6/PA7 总线 |
| 02 | bh1750 | sensor--bh1750-light-intensity-sensor.md | 0x23（0x46/0x47，ALT ADDRESS 接地） | `BH1750_SCL_GPIO/...` | `bh1750_init`（Power On 0x01）+ `bh1750_start_measure`（0x10 连续高分辨率）+ `bh1750_read_lux(lux)`（读地址 0x47 → 2B 高前低后 → /1.2；0=成功/1=无应答） | 页面 GY30_/IIC_/Single_Write_BH1750/Multiple_read 菜市场命名收敛；0x10 由 start_measure 独立暴露（页面散在 main）；等待 ≥BH1750_MEASURE_DELAY_MS=140ms 由调用方（模块宏）；页面缺陷 pages 待 facts 回填；默认不照抄 → PA6/PA7 总线 |
| 03 | sht20 | sensor--sht20-temp-humi-sensor.md | 0x40（0x80/0x81） | `SHT20_SCL_GPIO/...` | `sht20_init`（空占位——单次模式无预置命令）+ `sht20_read(t,h)`（0xF3 温度/0xF5 湿度各一段：写地址→写命令→读地址应答重试 ≤50×2ms→2B+NACK；raw&0xFFFC；温度 = raw/65536×175.72−46.85、湿度 = raw/65536×125−6；0=成功/1=温度段失败/2=湿度段失败） | **14bit 低 2 位状态位 &0xFFFC 掩码修正**（页面正文要求、页面代码未做——SHT2x 手册）；页面 do-while 裸轮询 → ≤50×2ms（覆盖 85ms）；无 CRC 按页面（校验和可以不需要，NACK 收尾）；与 pca9685 同址 0x40 提醒（notes，共总线同选冲突）；默认不照抄 → PA6/PA7 总线 |
| 04 | sht30 | sensor--sht30-temp-humi-sensor.md | 0x44（0x88/0x89，ADDR 接地） | `SHT30_SCL_GPIO/...` | `sht30_init`（0x2130 周期模式每秒 1 次高重复）+ `sht30_read(t,h)`（0xE000 读命令 → 读地址重试 ≤20×2ms → 6B = 温度高/低+CRC+湿度高/低+CRC → CRC8（0x31/0xFF）两组 → 温度 = d/65535×175−45、湿度 = d/65535×100；0=成功/1-5=页面失败码）+ `sht30_read_temperature/read_humidity` | 页面 SHT31_Write_mode 注释掉 IIC_Stop（重复起始）按页面原样；页外 extern 温度湿度全局 → 出参；页面失败码 1-5 保留；默认不照抄 → PA6/PA7 总线 |
| 05 | at24c02 | control--at24c02-eeprom-memory.md（control 目录） | 0x50（0xA0 写/0xA1 读） | `AT24C02_SCL_GPIO/...` | `at24c02_init`（空占位）+ `at24c02_write_byte(addr,data)` + `at24c02_read_byte(addr)`（伪写定位；无应答返回 0xFF）+ `at24c02_wait_write_done`（~5ms）+ `at24c02_write_page(addr,data,len)`（≤16B 页内、跨页拒收；0=成功/1=参数错/2=无应答）+ `at24c02_read_block(addr,buf,len)`（连续读：首字节 ACK 末字节 NACK、256 边界翻转；0=成功/1=参数错/2=无应答） | **页面地址宏 READ/WRITE 语义颠倒缺陷**（0xA0=写、0xA1=读——页面宏名互换）→ 命名纠正、值保留页面原式；页写/连续读按页面正文实现（页面仅字节读写代码）；写周期延时封装 wait_write_done（页面演示 delay_ms(5)）；WP 写保护由硬件接线管理不落码；默认不照抄 → PA6/PA7 总线 |
| 06 | ags10 | sensor--ags10-harmful-gas-sensor.md | 0x1A（0x34 写/0x35 读） | `AGS10_SCL_GPIO/...` | `ags10_init`（空占位——无初始化序列，预热 ≥120s 归调用方）+ `ags10_read(voc_ppb)`（写寄存器 0x00 → 读地址重试 ≤50×1ms → 5B = 状态 + TVOC 24bit + CRC → CRC8 校验 → TVOC = d[1..3] ppb；0=成功/1=通信失败/2=发送失败/3=等待超时/4=CRC 失败） | **页面读地址重试条件写反**（`while((WaitAck()==1) && (timeout >= 50))`——循环一次即退、超时分支永不触发）→ 修正 `timeout < 50`（mspm0 版同修）；**页面返回码与 TVOC 值混用**（ags10_read 既返回 TVOC 又返回 1-4 错误码——TVOC=1ppb 语义冲突）→ 出参 + 状态码（mlx90614 先例）；页面规格 I2C ≤15kHz 与页面代码时序矛盾——按代码（5us 半周期），真机异常调慢（唯一时序宏点）；默认不照抄 → PA6/PA7 总线 |
...
- 页内事实（代码块结构/F1/F4/极性/缺陷/API 对照）= 研究子代理报告（%TEMP%\batch2-facts.md），已回填 spec 决策表 + 工单。
