...
- [x] 推荐结果为空（无模块/无需求建议/无互斥组卡）时，步骤 5 保持未完成，步骤卡与总览不出现✓，空结果提示文案保留并指向模块库
- [x] 推荐结果有可用命中时，步骤 5 仍正常标记完成（含手动加模块场景不反向补标步骤 5）
- [x] 按默认布线生成（有引脚角色、未显式绑定、实例未配引脚）后，步骤 7 卡徽章与总览 chip 显示「默认布线」中性标记，而非「已就绪」✓
- [x] 显式绑定引脚/实例配脚/无角色需配置三种情形仍显示「已就绪」✓，行为不变
- [x] 新增纯函数与 node:test 单测覆盖上述四态判定
- [x] 全量前端测试与既有 01-09 之外所有用例保持绿
