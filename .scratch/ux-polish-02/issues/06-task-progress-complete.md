...
- [x] doing 态任务卡渲染 spinner + 阶段文案槽，执行中阶段切换（task_executing/compile_start/fix_start/task_reporting 等）实时更新该槽
- [x] 面板顶部单条状态行保留（汇总语义），执行结束后卡内槽清理
- [x] 全部任务处于终态（verified/skipped 之外无其他状态）时，总览/状态行显示「全部完成」+「去交付」按钮
- [x] 「去交付」点击切到交付页签（复用既有页签切换），无任务时按钮不出现
- [x] 渲染重建后：卡上展开中的 details 保持展开（按任务 id 恢复），用户未展开的不误弹
- [x] 新增纯函数单测：完成态判定（含 unverified/doing/failed/pending 不算完成）、折叠快照/恢复辅助
- [x] 既有下一步引导（做完一步 → 下一卡提示）与完成引导不冲突
