...
- [ ] `LOCAL_LLM_METHODS` 由 3 → 6：+clarify / validate_module_description /
      reference_judge_archivable。
- [ ] 派发更新：「落 local 的方法集 = 常量」行为断言（02 已建）随集合更新；
      每个新方法一条 fake 记录派发用例（落 local）。
- [ ] 未扩方法仍落 remote 断言：select_modules / generate_main_skeleton /
      generate_smoke_main / fix_compile_errors / distill_master /
      topic_split_topics / topic_extract_number。
- [ ] 真机复核：配置本地端点，真实跑一次澄清 + 简介校验 + 归档判定，输出
