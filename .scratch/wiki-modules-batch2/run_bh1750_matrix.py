# -*- coding: utf-8 -*-
"""bh1750 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "bh1750.h"\n'
    '#include "delay.h"\n'
    "int main(void) { bh1750_init(); uint8_t ok = bh1750_start_measure(); "
    "delay_ms(BH1750_MEASURE_DELAY_MS); float lux = 0; "
    "ok = bh1750_read_lux(&lux); (void)ok; (void)lux; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch2" / "matrix" / "bh1750"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["bh1750"])
generate(
