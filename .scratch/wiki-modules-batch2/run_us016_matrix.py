# -*- coding: utf-8 -*-
"""us016 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "us016.h"\n'
    "int main(void) { us016_init(); float d = us016_read_distance_cm(); "
    "(void)d; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch2" / "matrix" / "us016"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["us016"])
generate(
