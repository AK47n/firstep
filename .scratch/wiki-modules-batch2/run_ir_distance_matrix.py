# -*- coding: utf-8 -*-
"""ir_distance 编译矩阵：单选生成 → SysConfig CLI → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    "int main(void) { ir_distance_init(); float d = ir_distance_read_distance_cm(); "
    "(void)d; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch2" / "matrix" / "ir_distance"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ir_distance"])
generate(
