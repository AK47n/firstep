# -*- coding: utf-8 -*-
"""dht11 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "dht11.h"\n'
    "int main(void) { dht11_init(); float t = 0, h = 0; "
    "uint8_t ok = dht11_read(&t, &h); (void)ok; (void)t; (void)h; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch2" / "matrix" / "dht11"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["dht11"])
generate(
