...
- **2026-09-19 第三轮复核（本轮结论：仍核不出，挂账，不阻塞 v1.2.2 发版）**：
  发 v1.2.2 时用户点了这张单进来，于是又实测了一轮 Web 取源，结论与 04 续一致——
  **可核实的页面仍然不存在**，命中的仍是内容站 / 教程页 / 元件级商品：
  | slug | 本轮命中的候选 | 为什么仍**不算**（裁决一三条） |
  |---|---|---|
  | `beep` | EasyEDA 元件页（YL-44 BUZZER MODULE）、TELESKY 蜂鸣器模块文档页 | 前者是**元件库**条目（不是可购买/官方产品页），后者是资料站转载；极性（有源/低电平触发）也未标明 |
  | `ir_beam` | DFRobot 商城「红外对射光电开关」 | 形状对（3 线制对射），但**未核实**是否与库内实现同型号/同极性；且 04 续已把「非电赛小模块」一类排除，须人工确认实物 |
  | `key` / `led` | 仍只有入门教程页与不同硬件页 | 与裁决二冲突（教程页 = 板载资源，不是独立模块件） |
  | `led_beep` | 无 | 随 `led` / `beep` |
  | `step_motor` | 只有 B 站视频介绍（DCC-100v2/v3） | 内容站，不是官方或采购页；`dccmotor.com` 仍只有公司介绍 |
  **判定**：这 6 条卡在「要的是**用户实际买过的那件实物**的链接」这一步，机器代不了——
  继续按工单原意留在人工队列。**本轮把它显式记为不阻塞发版**（见
  `.scratch/release-v1.2.2/spec.md` 的「范围外」）。
- **为什么要单独开 06 而不是重开 04**：04 的交付物（待补清单 + 逐条依据 + 复跑探针）确实
