...
- [x] 验收 1：IDE 编译失败（目录 = 生成上下文）→ 点按钮 → 切到生成页修复
  中心 → 自动进入修复循环（AI 行动横幅出现、编译/修复事件照常流转）。
- [x] 验收 2：代码栏有未保存编辑 → 跳转后先弹写盘守卫「保存全部并继续 /
  取消」；取消 → 不开始循环。
- [x] 验收 3：无工具链 / 平台未选 → 按钮不可用或跳过自动开始（与手动入口
  同判据，不引入新状态）。
- [x] 验收 4：修复循环完成后 IDE 侧（若已打开同目录）可经 02/03 感知到写盘
  变更（集成不回归）。
- [x] 验收 5：CDP 冒烟覆盖主路径；全量回归绿。
