# -*- coding: utf-8 -*-
"""oled I2C 变体编译矩阵（回归）：单选生成 → SysConfig CLI 校验 → gmake 真编译。

批次 12/07 决策 B 零回归验证：OLED_Init（I2C1 硬件 I2C 路径）在 SPI 变体
落地后仍 0 error/0 warning 真编译。
"""
import sys
...
MAIN_C = (
    ...
    "    OLED_Init();\n"
    "    OLED_Clear();\n"
    "    OLED_ShowString(0, 0, (u8 *)\"I2C OK!\", 16);\n"
    "    OLED_Refresh();\n"
    "    while (1);\n"
    "}\n"
)

out = REPO / ".scratch" / "wiki-modules-batch12" / "matrix" / "oled_i2c"
if out.exists():
    import shutil
    shutil.rmtree(out)
