...
- [x] 保存触发：Ctrl/Cmd+S 全局监听（codeTabActive 才 preventDefault）+ 保存按钮；保存中按钮禁用 + 「保存中…」；450ms 内重复触发合并（防抖或 in-flight 判定）。
- [x] 成功路径：toast('ok')、脏点清除、大纲刷新（响应的 outline 直用）、codeFiles 里 size_bytes 更新 + renderCodeTree、tab.savedContent/mtime_ns 更新。
- [x] 失败路径：400 → toast('error', 中文)；网络/≥500 → 可重试（脏点保留）。
- [x] 非 UTF-8 只读：保存尝试 → toast 中文说明；无脏点（02 已标只读）。
- [x] 后端 409 在本票走「带 status 的错误 → toast 中文」占位（04 前不弹模态）。
- [x] smoke-03：编辑 → Ctrl+S → 服务端断言文件内容已写盘 + 页面 toast/脏点清除/大纲刷新；pytest 与 node --test 全绿。
