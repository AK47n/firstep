...
- [ ] 后端：覆盖写盘通道契约明确（服务端 `overwrite: true` 跳过 conflict 判定或重发携带新 mtime——实现期择一并注释）。
- [ ] `fx/codeeditor.js` 增 `conflictHTML(diskText, editText)` 纯函数（双列 pre 各前 10 行 + 越界省略号 + 动作按钮 data-action）。
- [ ] `ui/codeeditor.js` 冲突模态：409 呈现、三动作接线（覆盖 → 保存成功路径；重载 → GET 刷新 + 无脏点；取消 → 保持脏点）。
