...
- [x] .md 两态：预览态「编辑源码」按钮（仅活动 tab 为 .md 且 mdMode=预览 时可见）；切编辑态后 Ctrl+S 可保存；「返回预览」逻辑随 tab 切换保持正确（每 tab 各自 mdMode）。
- [x] 跳行联动：预览态下大纲/搜索/文件内查找命中 .md → 自动切编辑态并选中跳行。
- [x] main.c 联动：保存成功且 `isMainCPath(path) && codeDir === getMainCDiskDir()` → `refreshMainCDiskState()`；不引入循环 import（generate-mainc-sync 已在 codeview.js 引用）。
- [x] 文案更新：`#tab-code` h2、nav `data-tab="code"` title、中栏空态与只读相关措辞、树/侧栏无改动。
- [x] smoke-05：.md 编辑态往返、main.c 保存后步骤 8 状态行出现「已加载为编辑内容」类差异提示、文案快照。
- [x] 全量 pytest + tests/js 全绿；提交信息中文。
