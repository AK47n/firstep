...
- [ ] `ModuleManifest` 新增一个可选字段，表达「Python 副产物」声明（模板文件路径 + 输出文件名）；缺省 = 无（`None`/空）
- [ ] `to_dict` 在缺省时**不落键**，存量 manifest 序列化产物与基线逐字节一致（照 `multi_instance` 先例）
- [ ] `from_dict` 严格校验非法值（非对象 / 字段缺失 / 模板路径不安全或非相对）——错值大声失败，不静默强转
- [ ] 新增单元测试：解析 / 序列化往返、缺省兼容（旧 manifest 无该键仍加载）、非法值报错
- [ ] 全量测试绿 + mypy 干净（本工单不该让任何既有测试红）
