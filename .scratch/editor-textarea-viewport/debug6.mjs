...
console.log("before:", JSON.stringify(await Eval(`window.__taDebug()`)));
const hlDump = await Eval(`(() => {
  const hl = document.querySelector('#code-viewer .code-hl');
  const spacers = hl.querySelectorAll('.code-window-spacer');
  const lines = hl.querySelectorAll('.code-hl-line');
  return { childCount: hl.children.length, spacerCount: spacers.length,
    firstChild: (hl.children[0] ? hl.children[0].outerHTML.slice(0, 120) : null),
    secondChild: (hl.children[1] ? hl.children[1].outerHTML.slice(0, 80) : null),
    lineCount: lines.length,
    totalH: hl.scrollHeight, clientH: hl.clientHeight };
})()`);
console.log("hl before scroll:", JSON.stringify(hlDump));
...
console.log("after:", JSON.stringify(await Eval(`window.__taDebug()`)));
const after = await Eval(`(() => { const box=document.getElementById('code-viewer'); const ta=box.querySelector('.code-ta');
  const sp=box.querySelector('.code-hl .code-window-spacer');
  return { scrollTop: box.scrollTop, top: ta.style.top, spacerH: sp?sp.style.height:null }; })()`);
console.log("dom:", JSON.stringify(after));
