// debug-bf：bigfold.c 折叠+窗口手打 Q → undo 的逐步 dump（定位撤销失败）
...
const dump = (tag) => Eval(`(async () => {
  ...
  const m = tab ? tab.content : '';
  return { tag: ${JSON.stringify(tag)}, path: tab && tab.path, modelLen: m.length,
    modelQ: m.includes('Q'), sel: ta ? [ta.selectionStart, ta.selectionEnd] : null,
    taLen: ta ? ta.value.length : -1, taHead: ta ? ta.value.slice(0, 24) : '',
    hasPh: ta ? ta.value.includes('…') : false };
})()`);
...
await step("dump jump", () => dump("jump"));
await step("dbg jump", () => Eval(`window.__dshDbg ? window.__dshDbg() : 'no'`));
await step("fold toggle", () => Eval(`(() => { const ta = document.querySelector('#code-viewer .code-ta'); ta.focus(); ta.dispatchEvent(new KeyboardEvent('keydown', { key: '[', ctrlKey: true, shiftKey: true, bubbles: true, cancelable: true })); return true; })()`));
...
await step("dbg folded", () => Eval(`window.__dshDbg ? window.__dshDbg() : 'no'`));
...
await step("dump caret", () => dump("caret"));
await step("type Q", () => cdp("Input.insertText", { text: "Q" }));
await step("dump typed", () => dump("typed"));
await step("dbg typed", () => Eval(`window.__dshDbg ? window.__dshDbg() : 'no'`));
...
await step("dump undone", () => dump("undone"));
await step("dbg undone", () => Eval(`window.__dshDbg ? window.__dshDbg() : 'no'`));
process.exit(0);
