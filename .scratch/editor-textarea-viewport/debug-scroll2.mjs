// 调试2：监听页面异常，滚动后观察 ta 几何（02 现场排查）
...
ws.onmessage = (ev) => {
  ...
  if (msg.method === "Runtime.exceptionThrown") {
    events.push("EXC: " + (msg.params.exceptionDetails.exception?.description || JSON.stringify(msg.params.exceptionDetails)).slice(0, 500));
  }
  if (msg.method === "Runtime.consoleAPICalled") {
    events.push(msg.params.type + ": " + msg.params.args.map((a) => a.value ?? a.description ?? "").join(" ").slice(0, 300));
  }
};
...
await Eval(`window.addEventListener('error', (e) => console.error('PAGEERR', e.message, e.filename, e.lineno)); true`);
...
const after = await Eval(`(() => { const box=document.getElementById('code-viewer'); const ta=box.querySelector('.code-ta');
  ...
  return { scrollTop: box.scrollTop, top: ta.style.top, h: ta.style.height, taLen: ta.value.length, spacerH: sp?sp.style.height:null }; })()`);
