...
- [x] 探针：6000 行 .c 文末字母输入（词高亮活跃）复测 **29.8ms → 21.2–24.7ms**
      （从 ~38.9–45.8ms；多轮采样有机器噪声，达标 ≤30ms 目标）；
      **静态缓存（引导线/彩虹）修补不再依赖 markClean**——词/查找/错误活跃时
      同样增量（此前回删/词活跃的每次输入触发全库 bracketDepthMarks 扫描，
      本轮已消除，采样中消失）；纯字符路径与对齐矩阵/冒烟不回退。
