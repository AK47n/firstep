// 局部高清截图（code-editor-opt）：标签条 / 编辑器 / 状态栏 / 侧栏 / 底部面板
// CDP 9251 + webapp 8000；仅写 png。
...
const shotSel = async (name, selector) => {
  ...
  const shot = await cdp("Page.captureScreenshot", {
    format: "png",
    clip: { x: m[0], y: m[1], width: m[2], height: m[3], scale: 2 },
  });
...
for (const theme of ["light", "dark"]) {
  ...
  await shotSel("tabs-" + theme + ".png", "#code-tabs");
  await shotSel("editor-" + theme + ".png", "#code-viewer");
  await shotSel("sidebar-" + theme + ".png", "#code-tree");
  await shotSel("status-" + theme + ".png", ".code-status");
  await shotSel("bottom-" + theme + ".png", "#code-bottom");
}
process.exit(0);
