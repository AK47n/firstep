# -*- coding: utf-8 -*-
"""pca9685 stm32 编译矩阵（wiki-stm32-batch3/06）：单选生成 → UV4 真编译
（0 error/0 module warning 硬门槛 = exit 0）。照 wiki-stm32-batch2
run_aht10_matrix.py 配方改 slug；产物在 .scratch/wiki-stm32-batch3/matrix/pca9685/。"""
import sys
...
out = REPO / ".scratch" / "wiki-stm32-batch3" / "matrix" / "pca9685"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_STM32, ["pca9685"])
generate(
