...
- [x] 验收 1：webapp.py `/api/compile` 的 platform 改为可省略——缺省调 context_manifest._infer_platform(output_dir)（单源）；docstring 同步说明「platform 可省略，缺省自动推断」。
- [x] 验收 2：ContextError（两者都有/都没有 → 400 中文）在 errors.py 已登记且顺序正确（未登记则补登记）；显式传 platform 时行为与现状完全一致（回归）。
- [x] 验收 3：pytest——省略 platform 推断 stm32 一例、mspm0 一例；双平台/无平台配置 → 400 中文；显式 platform 回归；全量 pytest 绿。
- [x] 验收 4：events.py 的 compile done 契约无变化（platform 只是请求侧可选），若 docstring 有契约描述同步更新。
