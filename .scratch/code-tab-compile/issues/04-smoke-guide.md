...
- [ ] 验收 1：smoke.mjs 断言编译按钮存在 + 点击后出现成功状态行或错误列表；有错误时点错误行 → 目标 tab 打开 + selectionStart/End 与预期偏移一致（复用 compile-error-jump/smoke.mjs 探针模式）。
- [ ] 验收 2：guide-refs 代码栏小节补「编译」一句（与 guide.test.mjs / guide-refs.test.mjs 契约一致）。
- [ ] 验收 3：smoke-02~12 全绿 + node tests/js 全量绿 + pytest 全量绿。
- [ ] 验收 4：脚本 .ps1（如有）UTF-8 BOM；提交/工单/CHANGELOG 中文。
