...
- [ ] `smoke-09.mjs` 5000 行回车/Tab 同步耗时仍 < 50ms 均值（性能不退化）。
  **本轮实测**：逐键 **3.4ms** / Tab **10.0ms** 已达标（修复前 55.8 / 63.3）；**回车 76.1ms 仍差**
  （阈值 50，峰值 97.2 < 150 PASS）。**与本工单无关**——未折叠箭头只在结构变更时重建 gutter 字符串
  （`winBuild` 本来就要走）。本轮定位并修复了两处强制布局（见下「本轮同会话修复」第 1、4 项）：
  ① `winApplySize` 宽度缓存被行数变化重置 → 每次回车重新探针测量 → 强制深布局 ≈50ms；
  ② `taApplyWindowStyle` 读 `box.clientHeight` 触发强制布局（逐键/Tab 的大头）。
  剩余 76ms = 结构性编辑必走的 `winBuild` 全量重建 + 高度写导致的整树重排，属窗口化渲染的
  结构编辑成本，需单独性能工单优化（不在本工单范围）。
