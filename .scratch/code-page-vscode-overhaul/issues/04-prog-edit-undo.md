...
- [ ] 深色主题下逐路径验证：Tab / Shift+Tab / Enter / 括号 / 删行 / 移动行 / 注释 / 替换 后 Ctrl+Z 撤销、Ctrl+Y 重做，文本与选区正确。
- [ ] 连续多次程序化编辑后可连续 Ctrl+Z 逐步回退（不清空历史）。
- [ ] 键盘直接输入（字符、Backspace）的撤销行为不回归。
- [ ] 纯件/胶水可测部分补测；冒烟覆盖编辑器内焦点切换。
