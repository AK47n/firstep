...
- 2026-09-09 在途盘点（第六轮，CDP 实跑）：本单 Answer 声称的「smoke-02 7/7 PASS」当时**跑不出来**——脚本 `setText` 直写 `ta.value` 后编辑器按模型光标重渲染，注入选区被覆盖成光标，多行选区坍缩 → 实跑 0/7（值大多对、光标/选区错）。**产品侧无缺陷**（纯件 12 例单测 + 修复注入姿势后 7/7 全绿）；脚本改法同 smoke-01：派发按键前重设选区 + 等渲染落定。实测 `node .scratch/code-page-vscode-overhaul/smoke-02.mjs` → **7/7 PASS**。
