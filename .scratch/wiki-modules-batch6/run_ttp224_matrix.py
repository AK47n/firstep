# -*- coding: utf-8 -*-
"""ttp224 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "ttp224.h"\n'
    "int main(void) { ttp224_init(); (void)ttp224_read(1); "
    "(void)ttp224_read(4); (void)ttp224_read_all(); while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch6" / "matrix" / "ttp224"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ttp224"])
generate(
