# -*- coding: utf-8 -*-
"""ds18b20 编译矩阵：单选生成 → gmake 真编译（0 error 硬门槛）。"""
import sys
...
MAIN_C = (
    ...
    '#include "ds18b20.h"\n'
    "int main(void) { uint8_t ok = ds18b20_init(); (void)ok; "
    "float t = ds18b20_read_temp(); (void)t; while (1); }\n"
)

out = REPO / ".scratch" / "wiki-modules-batch6" / "matrix" / "ds18b20"
if out.exists():
    import shutil
    shutil.rmtree(out)
out.mkdir(parents=True)

resolved = resolve_selection(REPO / "library" / "modules", PLATFORM_MSPM0, ["ds18b20"])
generate(
