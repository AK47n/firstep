...
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t int_Length, uint8_t float_Length)
{
	int temp1, temp2;
	temp1 = (int)Number;
	// 修复：负数的小数部分要取绝对值，否则转为uint32_t后显示异常
	if (Number < 0)
		temp2 = (int)((temp1 - Number) * pow(10, float_Length));
	else
		temp2 = (int)((Number - temp1) * pow(10, float_Length));
...
/**
  * @brief  OLED大字显示一个字符（16×32像素，2×缩放8×16字库）
  * @param  Line 行位置，范围：1~2（每行32像素高）
  * @param  Column 列位置，范围：1~8（每列16像素宽）
...
{
	uint8_t p, c;
	uint8_t start_page = (Line - 1) * 4;     // 0 or 4
	uint8_t start_col  = (Column - 1) * 16;  // 0, 16, ..., 112
	const unsigned char *font = OLED_F8x16[Char - ' '];

	for (p = 0; p < 4; p++)
	{
		// 确定当前page对应的原始字库字节范围
		// p=0: 原字库[0..3]的上半(bit0-3) → page0
		// p=1: 原字库[0..3]的下半(bit4-7) → page1
		// p=2: 原字库[8..11]的上半(bit0-3) → page2
		// p=3: 原字库[8..11]的下半(bit4-7) → page3
		uint8_t orig_base = (p / 2) * 8;      // 0 or 8
		uint8_t bit_shift = (p % 2) * 4;      // 0 or 4
		uint8_t buf[16];

		for (c = 0; c < 16; c++) buf[c] = 0;

		for (uint8_t i = 0; i < 8; i++)       // 8列 → 16列
		{
			uint8_t fb = font[orig_base + i];
			uint8_t doubled = 0;
		...
		OLED_SetCursor(start_page + p, start_col);
		for (c = 0; c < 16; c++)
		{
			OLED_WriteData(buf[c]);
		}
	}
}

/**
  * @brief  OLED大字显示字符串（16×32像素）
  * @param  Line 起始行位置，范围：1~2
  * @param  Column 起始列位置，范围：1~8
  * @param  String 要显示的字符串（最多8字符）
  * @retval 无
  */
void OLED_ShowStringBig(uint8_t Line, uint8_t Column, char *String)
{
	while (*String)
	{
		OLED_ShowCharBig(Line, Column, *String);
		Column++;
		String++;
	}
}
