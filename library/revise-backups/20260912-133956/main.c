...
/* ---- 巡线（灰度 PD + 弧线跟随）标定值：场地半圆弧 R=40cm、线宽 1.8cm ----
 * 量纲：编码器计数 / 10ms（与 pid 模块一致）。260 线 / 轮径 48mm 时，
 * 12 counts/10ms ≈ 0.7 m/s，单圈（≈5m 路径）用时余量足够，弧线上不易冲线。 */
#define XUNJI_BASE_SPEED       12.0f   /* 基础速度（直线 / 缓弯巡航） */
#define XUNJI_BASE_SPEED_MIN   5.0f    /* 降速下限：弯道与丢线时仍保留前进力矩，防止停在线上 */
#define LINE_PID_KP            1.10f   /* 巡线 PD：P 主纠偏（弧线上跟线刚度） */
#define LINE_PID_KI            0.0f    /* 巡线不用积分，避免丢线保持时积分饱和 */
#define LINE_PID_KD            0.20f   /* 少量 D 抑制弧线上的左右摆动 */
#define LINE_DEADBAND          0.6f    /* 偏差死区：中心附近不做微调，抑制直道抖动 */
#define LINE_PID_OUT_MAX       7.0f    /* 轮速差上限（限幅保护） */
#define LINE_CURVE_DECEL       0.10f   /* 弯道减速系数：|err| 越大 base 越低（弧线跟线） */

/* 连续丢线（全白）超时：降速爬行找线，而不是保持原速死等 */
#define LINE_LOST_SLOW_TICKS   20U     /* 连续 20×10ms = 200ms 全白 → 判定丢线 */
#define LINE_LOST_SPEED_SCALE  0.5f    /* 丢线后基础速度降为一半，低速回转找线 */
...
static uint16_t line_lost_cnt   = 0;     /* 连续全白（丢线）帧计数，超时降速找线 */
    ...
    switch (xunji_state) {
        case XUNJI_STOP:
            ...
            if (startup_ms < START_DELAY_MS) {
                ...
                point_arm_cnt  = 0;
                point_debounce = 0;
                line_lost_cnt  = 0;             /* 起步清丢线计数，避免上次残留降速 */
        ...
        case XUNJI_FOLLOW:
        {
            ...
            if (!all_white_detect()) {          /* 有线才更新；全白=丢线时沿用上次偏差 */
                line_last_error = err;
                line_lost_cnt   = 0;            /* 重新压线：丢线计时清零 */
            } else if (line_lost_cnt < LINE_LOST_SLOW_TICKS) {
                line_lost_cnt++;                /* 连续全白累计（封顶，不溢出） */
            }
            ...
            /* ---- 2b) 弯道减速：偏差越大基础速度越低（弧线跟随靠此压住投影）---- */
            base = XUNJI_BASE_SPEED *
                   (1.0f - LINE_CURVE_DECEL * (err_dead > 0.0f ? err_dead : -err_dead));
            if (base > XUNJI_BASE_SPEED) base = XUNJI_BASE_SPEED;

            /* ---- 2c) 连续丢线超时：降速爬行找线（不死等、不停车）---- */
            if (line_lost_cnt >= LINE_LOST_SLOW_TICKS) {
                base *= LINE_LOST_SPEED_SCALE;
            }
            if (base < XUNJI_BASE_SPEED_MIN) base = XUNJI_BASE_SPEED_MIN;
