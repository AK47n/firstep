...
/*
 * 声光提示系统任务：
 * 在 check_point_and_alert() 中根据里程阈值，越过 A/B/C/D 点时调用
 * led_beep_alarm(1, 100, 100) 提示一次。
 * 支持原有路径（path_mode==0）与替代路径（path_mode==1），并确保多圈清零后
 * 每圈各点仍可重新提示。每个路径阶段只提示一次，路径阶段切换后自动复位。
 */
static void check_point_and_alert(void)
{
    static path_phase_t last_phase = PHASE_AB;
    static uint8_t flagged = 0;

    /* 路径阶段切换时，允许下一阶段重新提示 */
    if (path_phase != last_phase) {
        last_phase = path_phase;
        flagged = 0;
    }
    if (flagged) return;
    ...
    if (path_mode == 0) {
        switch (path_phase) {
            case PHASE_AB: threshold = AB_DISTANCE_MM; break;  /* 到 B */
            case PHASE_BC: threshold = BC_DISTANCE_MM; break;  /* 到 C */
            case PHASE_CD: threshold = CD_DISTANCE_MM; break;  /* 到 D */
            case PHASE_DA: threshold = DA_DISTANCE_MM; break;  /* 回到 A */
            default: return;
        }
    } else {
        switch (path_phase) {
            case PHASE_AB: threshold = AB_DISTANCE_MM; break;  /* 到 B（第一次） */
            case PHASE_BC: threshold = BC_DISTANCE_MM; break;  /* 到 C */
            case PHASE_CB: threshold = ALT_CB_MM; break;       /* 到 B（第二次） */
            case PHASE_BD: threshold = ALT_BD_MM; break;       /* 到 D */
            case PHASE_DA: threshold = ALT_DA_MM; break;       /* 回到 A */
            default: return;
        }
    }
...
static void xunji_dispatch(void) {
    ...
    switch (xunji_state) {
        ...
        case XUNJI_FOLLOW:
        {
            ...
            // 每经过 A/B/C/D 点时声光提示一次（由 check_point_and_alert 统一处理）
            ...
            if (state_timeout_ms > STATE_TIMEOUT_MS) {
                ...
                xunji_state = XUNJI_STOP;
                xunji_set_speed(0, 0);
                led_beep_alarm(1, 100, 100); // 停车声光提示
            }
            break;
        }
