...
// B→C 半圆弧段累计里程
#define BC_DISTANCE_MM   (AB_DISTANCE_MM + 3.141592653589793f * ARC_RADIUS_MM)
// C→D 段累计里程（底部直线长 2200mm）
#define CD_DISTANCE_MM   (BC_DISTANCE_MM + 2200.0f)
// D→A 半圆弧段累计里程，即完成一圈的总里程
#define DA_DISTANCE_MM   (CD_DISTANCE_MM + 3.141592653589793f * ARC_RADIUS_MM)

/* 状态切换时重置超时计数 */
static xunji_state_t last_state = XUNJI_STOP;
static uint32_t state_timeout_ms = 0;

// 完成一圈停车标志：置 1 后禁止再自动启动
...
static void xunji_dispatch(void) {
    ...
    switch (xunji_state) {
        ...
        case XUNJI_FOLLOW:
        {
            ...
            // 每经过 A/B/C/D 点时声光提示一次
            check_point_and_alert();

            // 路径状态机：按里程阈值切换 A→B → B→C → C→D → D→A → 回到A停车
            float avg_mm = (total_left_mm + total_right_mm) * 0.5f;
            ...
            switch (path_phase) {
                ...
                case PHASE_DA:
                    if (avg_mm >= DA_DISTANCE_MM) {
                        ...
                        xunji_set_speed(0, 0);
                        // 到达 A 点的声光提示已由 check_point_and_alert() 完成
                    }
                    break;
