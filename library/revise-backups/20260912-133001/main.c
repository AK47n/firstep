...
static void xunji_dispatch(void) {
    ...
    switch (xunji_state) {
        ...
        case XUNJI_FOLLOW:
        {
            ...
            /* ---- 4) 内环（编码器 → PWM）预留：本文件不调用，避免猜测电机 ID ----
             * TODO: 在 10ms 定时 ISR（或本调度内）执行：
             *   int32_t enc_l, enc_r;
             *   motor_encoder_read(&enc_l, &enc_r);
             *   motorA.now = (float)enc_l;   motorB.now = (float)enc_r;
             *   pid_cal(&motorA);  pid_cal(&motorB);
             *   motor_set_duty(<电机1 ID>, (uint32_t)limit_duty((int)motorA.out));
             *   motor_set_duty(<电机2 ID>, (uint32_t)limit_duty((int)motorB.out));
             * 电机 1/2 的 ID 宏在 motor.h 中未给出，确认后再放开。
             * 若 pid 模块内部已自带该定时环，本段直接删除。
             */
...
int main(void) {
    ...
    /* TODO: SYSCFG_DL_init() —— 调用了所选模块接口中不存在的函数 SYSCFG_DL_init，已注释占位，请改用真实接口或自行实现 */;
    ...
    gray_init();                /* 灰度巡线传感器 */
    motor_init(1);              /* 直流电机 1（PWMA/AIN 组）—— TODO: 确认 ID 与左右轮对应 */
    motor_init(2);              /* 直流电机 2（PWMB/BIN 组）—— TODO: 确认 ID 与左右轮对应 */
