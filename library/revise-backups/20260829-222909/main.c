...
#define XUNJI_FOLLOW_TIMEOUT_MS (15 * 1000)   // FOLLOW 状态超时：15s

// A->B 行驶距离阈值（约100cm）
#define XUNJI_TARGET_DISTANCE_CM (100.0f)
// 编码器平均脉冲换算距离（cm/脉冲）
...
static void xunji_dispatch(void) {
    // 累计里程变量：从 A 点出发的行驶距离（cm）
    static float xunji_total_distance_cm = 0.0f;
    // FOLLOW 状态超时计数器（10ms 调度周期累加）
    static uint32_t follow_timeout_ms = 0;
    ...
    switch (xunji_state) {
        ...
        case XUNJI_FOLLOW:
            // 前向约束超时保护：FOLLOW 状态持续超过预设时间未转入 WAIT_LINE，强制停车并切换 STOP
            follow_timeout_ms += 10;   // 调度周期 10ms，按实际节拍调整
            if (follow_timeout_ms >= XUNJI_FOLLOW_TIMEOUT_MS) {
                motor_set_duty(0, 0);
                motor_set_duty(1, 0);
                follow_timeout_ms = 0;
            ...
            // 2) 速度闭环：根据编码器反馈更新左右轮 PID 输出
            {
                ...
                // 累计里程：平均脉冲换算距离（A→B 任务）
                float avg_pulse = ((float)left_enc + (float)right_enc) * 0.5f;
                xunji_total_distance_cm += avg_pulse * DIST_PER_PULSE_CM;
            }
            pid_cal(&motorA);
            pid_cal(&motorB);

            // 3) 根据 PID 输出设置电机占空比与方向（只能前进：direction=0，负占空比截为 0）
            {
                ...
                motor_set_direction(0, (uint8_t)0);
                motor_set_duty(1, (uint32_t)duty_right);
                motor_set_direction(1, (uint8_t)0);
            }

            // 检测到行驶距离达到阈值（A→B 约100cm）→ 转入 WAIT_LINE 停车
            if (xunji_total_distance_cm >= XUNJI_TARGET_DISTANCE_CM) {
                follow_timeout_ms = 0;
