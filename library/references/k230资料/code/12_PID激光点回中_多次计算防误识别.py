...
try:
    ...
    pid_x = PID(-0.001, -0.002, 1.5/20*100, c_x)
    pid_y = PID(-0.001, -0.002, 1.5/20*100, c_y)
...
#        if key.value() == 1:
        ...
        if flag == 1:
            ...
            for i in range(5):
                ...
                img = img.gaussian(1)
                img = img.mean(2)
                blobs = img.find_blobs([(66, 76, 25, 64, -32, 11), (19, 74, 13, 91, -9, 48)], False,\
                                       (0, 0, 640, 640), x_stride=5, y_stride=5, \
                                       pixels_threshold=30, margin=False)
                for blob in blobs:
                    img.draw_rectangle(blob.x(), blob.y(), blob.w(), blob.h(), color=(0, 255, 0), thickness=2, fill=False)
                    c_x_lst.append(blob.x() + blob.w() / 2)
                    c_y_lst.append(blob.y() + blob.h() / 2)
                    img.compressed_for_ide()
                    Display.show_image(img)
                    break
            c_x_lst.sort()
            c_y_lst.sort()
            new_duty = pid_x.cal(sum(c_x_lst[1:-1])/len(c_x_lst[1:-1]))
            if new_duty > 2.5/20*100:
                new_duty = 2.5/20*100
            if new_duty < 0.5/20*100:
                new_duty = 0.5/20*100
            pwm_0.duty(new_duty)
            new_duty = pid_y.cal(sum(c_y_lst[1:-1])/len(c_y_lst[1:-1]))
            if new_duty > 2.5/20*100:
                new_duty = 2.5/20*100
            if new_duty < 0.5/20*100:
                new_duty = 0.5/20*100
