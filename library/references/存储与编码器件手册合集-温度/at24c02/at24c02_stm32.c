 ...
 * 2us ≈ 100kHz 级总线速度，AT24C02 400kHz 规格裕量充足；页面
 * `delay_us(1)/5/5` 混合半周期已统一为 2us——mspm0 版同款归一，notes 记录）。
 * SDA 方向切换：写 = 输出（SDA_OUT + 电平），读 = 输入（SDA_IN + 采样）；
 * 页面原式 GPIO_Mode_Out_OD（开漏输出）+ GPIO_Mode_IPU（上拉输入）→
 * ml_gpio OUT_OD/IU——总线需板上/模块自带上拉电阻。 */
#define AT24C02_SDA_OUT()  gpio_init(AT24C02_SDA_GPIO, AT24C02_SDA_PIN, OUT_OD)
#define AT24C02_SDA_IN()   gpio_init(AT24C02_SDA_GPIO, AT24C02_SDA_PIN, IU)
#define AT24C02_SDA_GET()  gpio_get(AT24C02_SDA_GPIO, AT24C02_SDA_PIN)
#define AT24C02_SDA(x)     gpio_set(AT24C02_SDA_GPIO, AT24C02_SDA_PIN, (x))
#define AT24C02_SCL(x)     gpio_set(AT24C02_SCL_GPIO, AT24C02_SCL_PIN, (x))
...
void at24c02_init(void)
{
     ...
     * 模块 API 一致；引脚配置 = 页面原式（OD + 两脚置高——页面无 SetBits，
     * 本实现置高 = 总线空闲电平，同 BH1750/SHT20 页惯例）；在线校验由
     * 读写判（无应答 = 未接/写周期中） */
    gpio_init(AT24C02_SCL_GPIO, AT24C02_SCL_PIN, OUT_OD);
    gpio_init(AT24C02_SDA_GPIO, AT24C02_SDA_PIN, OUT_OD);
    gpio_set(AT24C02_SCL_GPIO, AT24C02_SCL_PIN, 1);
    gpio_set(AT24C02_SDA_GPIO, AT24C02_SDA_PIN, 1);
}
...
uint8_t at24c02_write_page(uint8_t addr, const uint8_t *data, uint8_t len)
{
    ...
    if (data == 0 || len == 0 || len > AT24C02_PAGE_SIZE
