 ...
 *                与《1.28 寸圆屏/1.3 寸/1.47 寸/1.69 寸/1.8 寸触摸屏》
 ...
 * CS 高——片选空闲高；SCL 上升沿采样，SPI 模式 0；stm32 版 = gpio_set
 * 位操作——mspm0 版 DL_GPIO_setPins/clearPins 换算，零扩展字节尺寸
 * 差异；电平配置 = 页面原式 GPIO_Mode_Out_PP → ml_gpio OUT_PP）。
 * =========================================================================== */

#define LCD_SCL(x) gpio_set(LCD_SCL_GPIO, LCD_SCL_PIN, (x))
#define LCD_SDA(x) gpio_set(LCD_SDA_GPIO, LCD_SDA_PIN, (x))
#define LCD_RES(x) gpio_set(LCD_RES_GPIO, LCD_RES_PIN, (x))
#define LCD_DC(x)  gpio_set(LCD_DC_GPIO, LCD_DC_PIN, (x))
#define LCD_CS(x)  gpio_set(LCD_CS_GPIO, LCD_CS_PIN, (x))
#define LCD_BLK(x) gpio_set(LCD_BLK_GPIO, LCD_BLK_PIN, (x))



/* ====== 以下自 mspm0 lcd_init.c 移植（总线原语/序列表/模型表/状态——stm32 版仅总线层替换） ====== */
...
void lcd_init(uint8_t model, uint8_t dir)
{
    ...
    if (info->valid == 0 || info->seq == 0) { /* F1 头无 NULL（bmp180 注释口径） */
