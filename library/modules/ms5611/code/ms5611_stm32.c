 ...
 * 总线速度；页面原式 GPIO_Mode_Out_OD（开漏）+ GPIO_Mode_IPU（上拉输入）
 * → ml_gpio OUT_OD/IU（全批主流一派，模块板自带上拉电阻）；SDA 方向运行时
 * 切换：写 = 输出驱动，读 = 输入采样）。 */
#define MS5611_SDA_OUT()  gpio_init(MS5611_SDA_GPIO, MS5611_SDA_PIN, OUT_OD)
#define MS5611_SDA_IN()   gpio_init(MS5611_SDA_GPIO, MS5611_SDA_PIN, IU)
#define MS5611_SDA_GET()  gpio_get(MS5611_SDA_GPIO, MS5611_SDA_PIN)
#define MS5611_SDA(x)     gpio_set(MS5611_SDA_GPIO, MS5611_SDA_PIN, (x))
#define MS5611_SCL(x)     gpio_set(MS5611_SCL_GPIO, MS5611_SCL_PIN, (x))
 ...
 * 0 = 有应答、1 = 超时无应答（页面 10×5us）；页面 `char ack` 死变量已收敛） */
...
uint8_t ms5611_init(void)
{
    ...
    /* 引脚配置（批次 3/01 回修口径——F1 复位后 GPIO 浮空输入、ODR 写入
     * 无效：SCL 显式开漏输出 + 置高、SDA 开漏输出 + 置高） */
    gpio_init(MS5611_SCL_GPIO, MS5611_SCL_PIN, OUT_OD);
    MS5611_SCL(1);
    gpio_init(MS5611_SDA_GPIO, MS5611_SDA_PIN, OUT_OD);
    MS5611_SDA(1);
