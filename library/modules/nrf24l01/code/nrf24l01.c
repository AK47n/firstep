 ...
 * 页面：https://wiki.lckfb.com/zh-hans/dmx/module/rf/nrf24l01-2-4-g-control-module.html
...
/* NRF24L01 2.4G 无线收发（mspm0 纯驱动，软 SPI 位操作）：
 * 页内硬件 SPI（drv_spi.c 的 DL_SPI_*）改 GPIO 位操作（CLK/MOSI/MISO 走
 * NRF24L01 实例宏，零引脚字面量）；页内 30+ 函数收敛为库风格子集（寄存器
 * 原语静态化，对外 init / set_channel / tx / rx / flush）；`#if DYNAMIC_PACKET==0` 分支
 * 引用页外 L01_WriteSingleReg（上游残留 bug）——整枝剔除，动态包长归一为 1；
 * 页内多处 `NRF24L01_Write_Reg(FLUSH_* , 0xff)` 误把 FLUSH 命令当寄存器地址
 * （顺带多写一字节）——统一改用显式 flush 命令（CS 低 + 单命令字节）。 */

/* ---------- 软 SPI 原语（位操作，SPI 模式 0：上升沿采样） ---------- */

static void _cs_low(void)
{
    DL_GPIO_clearPins(NRF24L01_PORT, NRF24L01_CSN_PIN);
}

static void _cs_high(void)
{
    DL_GPIO_setPins(NRF24L01_PORT, NRF24L01_CSN_PIN);
}

static void _ce_low(void)
{
    DL_GPIO_clearPins(NRF24L01_PORT, NRF24L01_CE_PIN);
}

static void _ce_high(void)
{
    DL_GPIO_setPins(NRF24L01_PORT, NRF24L01_CE_PIN);
}

static uint8_t _irq_level(void)
{
    return (DL_GPIO_readPins(NRF24L01_PORT, NRF24L01_IRQ_PIN)
            & NRF24L01_IRQ_PIN) ? 1 : 0;
}
