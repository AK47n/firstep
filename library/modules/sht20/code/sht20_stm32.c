 ...
 * 5us ≈ 100kHz 级总线速度，SHT2x 规格 ≤400kHz，裕量充足；页面本页 2-6us
 * 混合变体——mspm0 版同款归一，notes 记录）。
 * SDA 方向切换：写 = 输出（SDA_OUT + 电平），读 = 输入（SDA_IN + 采样）；
 * 页面原式 GPIO_Mode_Out_OD（开漏输出）+ GPIO_Mode_IPU（上拉输入）→
 * ml_gpio OUT_OD/IU——总线需板上/模块自带上拉电阻。 */
#define SHT20_SDA_OUT()  gpio_init(SHT20_SDA_GPIO, SHT20_SDA_PIN, OUT_OD)
#define SHT20_SDA_IN()   gpio_init(SHT20_SDA_GPIO, SHT20_SDA_PIN, IU)
#define SHT20_SDA_GET()  gpio_get(SHT20_SDA_GPIO, SHT20_SDA_PIN)
#define SHT20_SDA(x)     gpio_set(SHT20_SDA_GPIO, SHT20_SDA_PIN, (x))
#define SHT20_SCL(x)     gpio_set(SHT20_SCL_GPIO, SHT20_SCL_PIN, (x))
...
static void sht20_iic_send_ack(uint8_t is_nack)
{
    SHT20_SDA_OUT();
    SHT20_SCL(0);
    SHT20_SDA(0); /* 页面原式：先置 0，再按 is_nack 重设（is_nack=0 时同值二次写） */
    delay_us(5);
    if (!is_nack) {
        SHT20_SDA(0);
    } else {
...
/* 等待从机应答（页面 IIC_Wait_Ack：0=有应答/1=超时无应答；超时发停止信号）。
 ...
 * 应答直到测量完成（no-hold 单次——页面 do-while 裸循环无上限改
 * ≤50×2ms 重试，覆盖页面 85ms/29ms 最长测量——缺陷①修正）→ 读 2 字节 +
 * NACK + 停止；
...
static uint8_t sht20_measure_once(uint8_t cmd, uint16_t *raw)
{
     ...
     * 页面代码未实现——按 SHT2x 手册修正 & 0xFFFC，缺陷②） */
...
{
    /* 单次测量模式无器件初始化序列（页面 bsp 无 init 函数——空实现占位，
     * mspm0 先例）；引脚配置 = 页面 SHT20_GPIO_INIT 原式（OD + 两脚置高 =
     * 总线空闲电平，页面 GPIO_SetBits 拉高） */
    gpio_init(SHT20_SCL_GPIO, SHT20_SCL_PIN, OUT_OD);
    gpio_init(SHT20_SDA_GPIO, SHT20_SDA_PIN, OUT_OD);
    gpio_set(SHT20_SCL_GPIO, SHT20_SCL_PIN, 1);
    gpio_set(SHT20_SDA_GPIO, SHT20_SDA_PIN, 1);
}
