/* 来源：立创开发板技术文档中心（wiki.lckfb.com）《MQ-4甲烷检测传感器》
 * 页面：https://wiki.lckfb.com/zh-hans/dmx/module/sensor/mq-4-sensor.html
...
/* MQ-4 甲烷/天然气检测传感器驱动（mspm0 纯驱动薄封装，ADR 0009）：
 * - 薄封装：依赖库内 adc 模块读 ADC12_0 MEM0（adc_get(ADC_1, ADC_Channel_0)），
 *   不新开 ADC 通道——mq4 与 adc/us016/mq2/批次 9 四件共享 MEM0 槽位
 *   （默认 PA24/A0_3）；
 ...
 * 对应手册：sources/materials/lckfb-地猛星移植手册/sensor--mq-4-sensor.md
 * （立创 wiki 地猛星移植手册；代码按模块库规范改写：去 main/printf、函数名
 * 规范化、ADC 中断改轮询、DO 数字量阈值未声明——页面 Get_MQ4_DO_value
...
/* mq4_init：使能 ADC12_0 转换（薄封装——adc_init 即 MEM0 轮询初始化）。 */
void mq4_init(void);
