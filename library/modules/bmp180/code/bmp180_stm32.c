...
#include "headfile.h"
#include "math.h" /* pow：海拔换算（ir_distance 先例——ARMCC 标准库自动含
                   * math，uvprojx 无需显式链接项） */

/* BMP180 软 I2C 位操作原语（立创 bsp 同款时序：SCL 半周期 5us ≈ 100kHz 级
 * 总线速度；页面电平配置 = GPIO_Mode_Out_OD（开漏输出）+ GPIO_Mode_IPU
 * （上拉输入）→ ml_gpio OUT_OD/IU（全批主流一派，模块板自带上拉电阻）；
 * SDA 方向运行时切换：写 = 输出驱动，读 = 输入采样）。 */
#define BMP180_SDA_OUT()  gpio_init(BMP180_SDA_GPIO, BMP180_SDA_PIN, OUT_OD)
#define BMP180_SDA_IN()   gpio_init(BMP180_SDA_GPIO, BMP180_SDA_PIN, IU)
#define BMP180_SDA_GET()  gpio_get(BMP180_SDA_GPIO, BMP180_SDA_PIN)
#define BMP180_SDA(x)     gpio_set(BMP180_SDA_GPIO, BMP180_SDA_PIN, (x))
#define BMP180_SCL(x)     gpio_set(BMP180_SCL_GPIO, BMP180_SCL_PIN, (x))
 ...
 * "Write_Cmd NACK -1/-2/-3"——页面仅 printf 不中断，本件按状态码中止） */
 ...
 * `(d0<<16|d1<<8|d2)>>8`（oss=0 时与标准 `>>(8-oss)` 一致——按页面保留，无
 * 掩码 & 0xFFFC——缺陷⑤反向守卫）；逐字节重发写地址，读地址应答重试 ≤5×1ms
 * （页面 timeout<5，观察：timeout 跨字节累计不归零——mspm0 同款保留，notes
 * 记录）；NACK printf 改状态码）。返回 0 = 成功、1 = 器件地址无应答、
 * 2 = 寄存器地址无应答、3 = 读地址应答超时（>5×1ms）；成功时经 *out 带回。 */
...
uint8_t bmp180_init(void)
{
    ...
    /* SCL 初始化（批次 3/01 回修口径：F1 复位后 GPIO 浮空输入、ODR 写入
     * 无效——总线空闲电平必须显式置高；页面 GPIO_Init 本身已 Out_OD +
     * SetBits（合规），stm32 版显式保留） */
    gpio_init(BMP180_SCL_GPIO, BMP180_SCL_PIN, OUT_OD);
    BMP180_SCL(1);
    gpio_init(BMP180_SDA_GPIO, BMP180_SDA_PIN, OUT_OD);
    BMP180_SDA(1);
...
uint8_t bmp180_read(float *temp_c, float *pa)
{
    ...
    /* 页面原式保留（B7 uint32_t——页面声明；**标准 BMP180 实现用 long B7
...
float bmp180_read_altitude(float pa)
{
     ...
     * 压强注释保留为公式常量；与 ms5611 共用 44330 公式——两件分工见
     * manifest notes） */
