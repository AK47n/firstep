...
volatile uint32_t g_zigbee_link_byte_count  = 0;  // 诊断：ZIGBEE_UART 收到的总字节数
...
void zigbee_link_send(const uint8_t *payload, uint8_t len)
{
    ...
    DL_UART_transmitDataBlocking(ZIGBEE_UART_INST, ZIGBEE_LINK_SYNC1);
    DL_UART_transmitDataBlocking(ZIGBEE_UART_INST, ZIGBEE_LINK_SYNC2);
    DL_UART_transmitDataBlocking(ZIGBEE_UART_INST, len);
    for (i = 0; i < len; i++)
        DL_UART_transmitDataBlocking(ZIGBEE_UART_INST, payload[i]);
