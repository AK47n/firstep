...
/* SYN6288E 中文语音合成（stm32 纯驱动，软 UART 单发 TX）：
 * 页内驱动（bsp_syn6288.c/h）走硬件串口（UART2 9600），本件按批次 9 定稿
 * 改软 UART 位操作（9600 8N1）；页面 SYN6288_Init 只开串口中断（RX 状态
 * 回传缓冲），随软 UART 裁剪——本件 init 只初始化 GPIO + 置空闲电平；
...
{
    gpio_set(SYN6288_GPIO, SYN6288_PIN, level);
...
{
    gpio_init(SYN6288_GPIO, SYN6288_PIN, OUT_PP); /* 语音输出脚推挽输出 */
    gpio_set(SYN6288_GPIO, SYN6288_PIN, 1);       /* 空闲高电平 */
}
