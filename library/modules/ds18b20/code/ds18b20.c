 ...
 * 页面：https://wiki.lckfb.com/zh-hans/dmx/module/sensor/ds18b20-temp-sensor.html
 ...
 * 落页面 60-70us 区间；常量单源在 ds18b20.h，源码不散写字面量）。
 * 数据线方向运行时切换：输出（拉低/释放总线）+ 输入（采样从机应答与数据位）。 */
 ...
 * 0x44（转换）；随后按数据手册等待 12bit 最大转换时间（750ms）。 */
...
{
    return ds18b20_check(); /* 复位检测器件（页面 DS18B20_Init/Check 语义） */
}
...
float ds18b20_read_temp(void)
{
    ...
    ds18b20_start_convert();                    /* 0xCC + 0x44 + 等待转换 */
