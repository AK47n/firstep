...
/* MQ-5 液化气/天然气传感器驱动（stm32 纯驱动薄封装，ADR 0009）：
 * - **ADC 共享组共读**：经母版 ml_adc API 读 ADC1 通道（mq5_init =
 *   adc_init(ADC_1, MQ5_AO_CH)；ADC 通道宏 MQ5_AO_CH 单源在 pin_config.h，
 ...
 * - 5 次快速平均（页面 SAMPLES 30 次累加，mq2 快平均先例改 5 次）；
 * - DO 数字量未声明（页面 Get_MQ5_DO_value（PA1，IPU）演示未用——mspm0
 *   先例「未用不声明」，不落 pins/不落码）。
 * 对应手册：sources/materials/lckfb-地阔星移植手册/sensor--mq-5-sensor.md
 * （立创 wiki 地阔星移植手册；代码按模块库规范改写：去 main/printf、函数名
 * 规范化、页面 ADC 序列收敛 ml_adc、SAMPLES 30→5 快平均、C99 循环声明改
 * uint8_t；页面 L187「酒精值」串台（MQ-3 模板残留）——notes 记录不落码）。 */
...
/* mq5_init：初始化 ADC1 对应通道（AO 模拟输入——页面 ADC_MQ5_Init 的
