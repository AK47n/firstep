...
/* MFRC522 射频 IC 卡识别（stm32 纯驱动，软 SPI 位操作）：
 ...
 * RC522_<PIN>_PIN）；延时走母版 ml_delay（页面 200us 半周期时序保留）；
...
static void _cs_low(void)  { gpio_set(RC522_PORT, RC522_CS_PIN, 0); }
static void _cs_high(void) { gpio_set(RC522_PORT, RC522_CS_PIN, 1); }
static void _rst_enable(void)  { gpio_set(RC522_PORT, RC522_RST_PIN, 0); }
static void _rst_disable(void) { gpio_set(RC522_PORT, RC522_RST_PIN, 1); }
static void _sck_low(void)  { gpio_set(RC522_PORT, RC522_SCK_PIN, 0); }
static void _sck_high(void) { gpio_set(RC522_PORT, RC522_SCK_PIN, 1); }
static void _mosi_low(void)  { gpio_set(RC522_PORT, RC522_MOSI_PIN, 0); }
static void _mosi_high(void) { gpio_set(RC522_PORT, RC522_MOSI_PIN, 1); }

static uint8_t _miso_level(void)
{
    return gpio_get(RC522_PORT, RC522_MISO_PIN) ? 1u : 0u;
}
...
/* （页面 _antenna_off 未在服务路径调用——不落，防 UV4 #177-D 未引用告警；
 *   mspm0 版保留原函数，stm32 版仅裁剪未引用静态，语义等价） */
...
{
    /* GPIO 引脚配置（CS/RST/SCK/MOSI 推挽输出 + MISO 上拉输入；ml_gpio 的
     * gpio_init 内部使能 RCC 时钟——页面 GPIO_Init 换算） */
    gpio_init(RC522_PORT, RC522_CS_PIN, OUT_PP);
    gpio_init(RC522_PORT, RC522_RST_PIN, OUT_PP);
    gpio_init(RC522_PORT, RC522_SCK_PIN, OUT_PP);
    gpio_init(RC522_PORT, RC522_MOSI_PIN, OUT_PP);
    gpio_init(RC522_PORT, RC522_MISO_PIN, IU);
    _cs_high();   /* 片选空闲高 */
    _rst_disable();
    _reset();
    _config_type_a();
