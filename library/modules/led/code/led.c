...
/* 地猛星用户 LED（PA15，LED_BEEP 组）：一脚接地、另一脚接引脚（拉电流），
 * 高电平点亮、低电平熄灭。板载 PA0/PA1 被 I2C0（MPU6050）占用。 */
...
{
    (void)channel;
    DL_GPIO_setPins(LED_BEEP_PORT, LED_BEEP_LED_PIN); // 高电平点亮
}

void led_off(uint8_t channel)
{
    (void)channel;
    DL_GPIO_clearPins(LED_BEEP_PORT, LED_BEEP_LED_PIN); // 低电平熄灭
}
