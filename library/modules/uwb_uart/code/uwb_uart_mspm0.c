...
// ============================================================
//  初始化：SYSCFG_DL_init() 已配好 UWB_UART（UART2, PA23/PA24,
//  115200, RX 中断），这里只开 NVIC 并复位状态机/滤波器。
// ============================================================
void uwb_uart_init(void)
{
    NVIC_ClearPendingIRQ(UWB_UART_INST_INT_IRQN);
    NVIC_EnableIRQ(UWB_UART_INST_INT_IRQN);
...
static void parse_frame(void)
{
    ...
    if (header != 0xFFFFFFFFUL) return;

    uint16_t cmd = swap16(uwb_buf.frame.cmd);
    if (cmd != 0x2001U) return;
    ...
    {
        int16_t az_abs = (g_uwb_raw.azimuth < 0)
            ? (int16_t)-g_uwb_raw.azimuth : g_uwb_raw.azimuth;
        int16_t cur_az = g_uwb_filtered.azimuth;
        int16_t diff = (int16_t)(az_abs - cur_az);
        if (cur_az > 0)  // 滤波器已有有效值才钳位
        {
            if (diff > AZ_MAX_STEP)
                az_abs = (int16_t)(cur_az + AZ_MAX_STEP);
    ...
    g_uwb_updated  = 1;
    g_uwb_frame_count++;
    // mspm0 无全局 g_systick：last_tick 记有效帧序号（毫秒超时请同选
    // ntb_time 用 get_time_stamp_ms——纯驱动不自行占定时器）
    g_uwb_last_tick = g_uwb_frame_count;
}
...
void uwb_rx_handler(void)
{
    ...
    switch (DL_UART_getPendingInterrupt(UWB_UART_INST))
    {
    case DL_UART_IIDX_RX:
        byte = DL_UART_receiveData(UWB_UART_INST);
        break;
    default:
        return;
    }
...
// UWB_UART_INST_IRQHandler = UART2_IRQHandler（母版 SysConfig 宏）
void UWB_UART_INST_IRQHandler(void)
{
    uwb_rx_handler();
}

// ============================================================
//  重置滤波器 (DIP ID 变更时调用，避免旧帧数据污染)
// ============================================================
