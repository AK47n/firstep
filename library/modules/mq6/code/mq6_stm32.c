...
/* MQ-6 液化气/丙烷检测（stm32 纯驱动薄封装）：
 * - 通道 = 母版 pin_config.h 单源 MQ6_AO_CH（默认 ADC_Channel_5 = PA5——
 ...
 * - 换算 percent = value/4095×100（页面 Get_MQ6_Percentage_value 原式——
 *   正向映射：对丁烷/丙烷/甲烷灵敏度高、特别适合液化气（丙烷）；相对值非
 *   ppm 精标：模块上电后 AO 电压随加热/环境/老化漂移，需预热、真实 ppm
 *   需标准气体标定）；
 * - 5 次快速平均（页面 SAMPLES 30 次 × delay_ms(5) 太慢——改 5 次快平均，
 *   mq2/us016/mspm0 版先例；无采样延时）；
 * - DO 数字量未声明：页面 Get_MQ6_DO_value（PA1，IPU）演示未用（LM393
...
void mq6_init(void)
{
     ...
     * 等效页面 ADC_MQ6_Init 流程——采样时间 239.5cyc vs 页面 55.5cyc，
     * 功能等价差异 notes 记录） */
    adc_init(ADC_1, MQ6_AO_CH);
}
...
float mq6_read_percent(void)
{
    ...
    for (i = 0; i < MQ6_ADC_SAMPLES; i++) {
        sum += adc_get(ADC_1, MQ6_AO_CH);
    }
    return (float)(sum / MQ6_ADC_SAMPLES) / (float)MQ6_ADC_MAX * 100.0f;
}
