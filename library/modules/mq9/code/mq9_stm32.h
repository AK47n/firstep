...
/* MQ-9 一氧化碳/可燃气体传感器驱动（stm32 纯驱动薄封装，ADR 0009）：
 * - **ADC 共享组共读**：经母版 ml_adc API 读 ADC1 通道（mq9_init =
 *   adc_init(ADC_1, MQ9_AO_CH)；ADC 通道宏 MQ9_AO_CH 单源在 pin_config.h，
 ...
 *   Get_MQ9_Percentage_value 原式——**相对值**，非 ppm 精标；模块上电
 *   需预热（几分钟级）、真实 ppm 需标准气体标定）；
 * - **器件双温循环原理**：低温 1.5V 测 CO、高温 5.0V 测可燃气体并清洗——
 *   但页面驱动仅单 AO 单路百分比、4Pin 模块无加热控制脚（双通道区分需模块
 *   级温控/标定，notes 记录；与 mq7（CO 专一）/mq6（可燃气专一）分工）；
 * - 轮询读取（无 ADC 中断——模块 API 忙等单点实现）；
 * - 5 次快速平均（页面 SAMPLES 30 次累加，mq2/us016 快平均先例改 5 次）；
 * - DO 数字量未声明（页面 Get_MQ9_DO_value（PA1，IPU）演示未用——mspm0
 *   先例「未用不声明」，不落 pins/不落码）。
 * 对应手册：sources/materials/lckfb-地阔星移植手册/sensor--mq-9-sensor.md
...
/* mq9_init：初始化 ADC1 对应通道（AO 模拟输入——页面 ADC_MQ9_Init 的
 * AIN 配置 + 6 分频 12MHz + 校准流程由 ml_adc 内部完成）。 */
void mq9_init(void);

/* mq9_read_percent：读 AO 电压对应浓度百分比（0-100% 相对值——MQ 系读数
 * 随环境/老化漂移，非 ppm 精标；模块预热后读数才稳定；对 CO/可燃气体灵敏；
 * 双温循环区分需模块级温控/标定——本件只读 AO 相对百分比）。 */
