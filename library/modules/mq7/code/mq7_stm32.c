...
/* MQ-7 一氧化碳检测（stm32 纯驱动薄封装）：
 * - 通道 = 母版 pin_config.h 单源 MQ7_AO_CH（默认 ADC_Channel_5 = PA5——
 ...
 *   正向映射：电导率随一氧化碳浓度增加而增大；相对值非 ppm 精标：模块上电
 *   后 AO 电压随加热/环境/老化漂移，需预热、真实 ppm 需标准气体标定）；
 * - **高低温循环检测说明**：器件为高低温循环检测（低温 1.5V 测 CO、高温
 *   5.0V 清洗），但 4Pin 模块 AO 单路输出、页面驱动仅单路百分比——本件只读
 *   AO 相对百分比（notes 记录）；
 * - 5 次快速平均（页面 SAMPLES 30 次 × delay_ms(5) 太慢——改 5 次快平均，
 *   mq2/us016/mspm0 版先例；无采样延时）；
 * - DO 数字量未声明：页面 Get_MQ7_DO_value（PA1，IPU）演示未用（LM393
...
void mq7_init(void)
{
     ...
     * 等效页面 ADC_MQ7_Init 流程——采样时间 239.5cyc vs 页面 55.5cyc，
     * 功能等价差异 notes 记录） */
    adc_init(ADC_1, MQ7_AO_CH);
}
