...
/* JQ8900-16P 语音播报（stm32 纯驱动，软 UART 单发 TX）：
 * 页内驱动（bsp_jq8900.c/h）走硬件串口（两线 9600 0xAA 帧）与一线 GPIO
 * 时序（SendData 3:1 脉宽）双描述，本件按批次 9 定稿学**两线帧**改软 UART
 * 位操作（9600 8N1）；页面 JQ8900_Init 只开串口中断（RX 缓冲/播放完成
 * 反馈），随软 UART 裁剪——本件 init 只初始化 GPIO + 置空闲电平；页面
 * SendData 是一线串行控制（GPIO APP 脚 3:1 脉宽）而非两线串口指令帧，
 * 不收纳（需要一线串行另立模块）；UART 命令帧结构按页面 demo
 * {0xAA,0x06,0x00,0xB0} 实证（0xAA + cmd + data + 校验和）。 */

/* 发送 1 位：电平 + 一个位周期（阻塞忙等，不占 TIMER） */
static void _tx_bit(uint8_t level)
{
    gpio_set(JQ8900_GPIO, JQ8900_PIN, level);
...
{
    gpio_init(JQ8900_GPIO, JQ8900_PIN, OUT_PP); /* 语音输出脚推挽输出 */
    gpio_set(JQ8900_GPIO, JQ8900_PIN, 1);       /* 空闲高电平 */
}
