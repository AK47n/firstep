...
/* 红外遥控接收（stm32 纯驱动，NEC 协议）：
 * - 接收头输出：无信号 = 高电平、载波脉冲 = 低电平（与手册 GET_OUT 一致）；
 * - 计时：CPU 忙等 + delay_us(20) 步进（20us 分辨率，560us 位宽约 28 拍；
 *   不占 TIMER——stm32 TIM 全被 PWM/编码器/骨架调度占用）；
 * - 触发：手册在 EXTI2 下降沿中断内同步解码整帧；本库改主循环轮询（等待
 *   空闲高 → 等待下降沿开始引导码 → 忙等解码）——stm32 EXTI 聚合/编码器
 *   独占先例（ir_beam 轮询先例），轮询与中断解码行为等价。
 * 阈值（20us 拍）：引导码低 400-500 拍（8-10ms）/高 100-250 拍（2-5ms）、
 * 重复码高 100-150 拍（2-3ms）；位低 20-60 拍（0.4-1.2ms）；位高 60-100 拍
 * = 1（1.2-2ms）、10-50 拍 = 0（0.2-1ms）——与手册完全一致（mspm0 同）。 */
...
{
    return gpio_get(IR_REMOTE_PORT, IR_REMOTE_OUT_PIN) ? 1 : 0;
}
...
{
    gpio_init(IR_REMOTE_PORT, IR_REMOTE_OUT_PIN, IU); /* 上拉输入（页面配置） */
    _have = 0;
    _repeat = 0;
    _address = 0;
    _code = 0;
