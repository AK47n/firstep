 ...
 * （sequence 六通道：MEM0=PA24/A0_3 归本模块（us016/mq2 薄封装共读同槽——
 ...
 * adc_get 通道 0-5 = MEM0-5，MEM1 与 joystick_read_x 同读一通道）。 */
...
uint16_t adc_get(ADCx_enum adc, ADCINx_enum adc_channel)
{
    ...
    if (adc_channel > ADC_Channel_5) {
        return 0; /* 本模块开放 MEM0-5：MEM0=本模块/us016/mq2（薄封装共读），
                   * MEM1=摇杆 X、MEM2=摇杆 Y、MEM3=ir_distance、MEM4=mq135、
                   * MEM5=mq5（共享实例同读——多路气体同选时物理通道独立） */
    }
