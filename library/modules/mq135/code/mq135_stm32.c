...
/* MQ-135 空气质量检测（stm32 纯驱动薄封装）：
 * - 通道 = 母版 pin_config.h 单源 MQ135_AO_CH（默认 ADC_Channel_5 = PA5——
 ...
 *   模式，页面 for(int i...)）；
 * - DO 数字量未声明：页面 Get_MQ135_DO_value（PA1，IPU）演示未用
...
void mq135_init(void)
{
     ...
     * 等效页面 ADC_MQ135_Init 流程——采样时间 239.5cyc vs 页面 55.5cyc，
     * 功能等价差异 notes 记录） */
    adc_init(ADC_1, MQ135_AO_CH);
}
