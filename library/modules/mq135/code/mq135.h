...
/* MQ-135 空气质量传感器驱动（mspm0 纯驱动切片，ADR 0009）：
 * - **独立 ADC 通道**：经库内 adc 模块 API 读 ADC12_0 MEM4（adc_get(ADC_1,
 *   ADC_Channel_4)，默认 PB20/A0_6，母版 sequence 五通道 endAdd=4）——
 *   与 mq2 的 MEM0 薄封装（adc/us016/mq2 共读同槽）不同：多路气体同选时
 *   各器件物理通道独立、无共读冲突（mq2+mq135 同选 = PA24/PB20 两槽位）；
 ...
 * 对应手册：sources/materials/lckfb-地猛星移植手册/sensor--mq-135-sensor.md
 * （立创 wiki 地猛星移植手册；代码按模块库规范改写：去 main/printf、函数名
 * 规范化、ADC 中断改轮询、DO 数字量阈值未声明——页面 Get_MQ135_DO_value
...
/* mq135_init：使能 ADC12_0 转换（独立 MEM4 通道轮询——adc_init 即初始化）。 */
