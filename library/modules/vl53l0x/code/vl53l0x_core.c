 ...
 *        STM32F103C8T6_ProjectTemplate/bsp/VL53L0X/core/ （ST 官方
 *        VL53L0X API：api/api_core/calibration，BSD-3-Clause 许可，
 *        文件头保留 ST 版权声明；使用 / 复制 / 修改 / 传播请遵循
 *        ST 许可与立创版权要求：标明来源与链接）。
 * 本文件 = 最小切片：按模块库「纯驱动切片」规范从官方 API 全套裁剪——
 * 只保留初始化/模式配置/单次测量传递闭包（DataInit/StaticInit/
 * PerformRefCalibration/PerformRefSpadManagement/SetDeviceMode/
 * SetLimitCheckEnable/SetLimitCheckValue/SetMeasurementTimingBudgetMicroSeconds/
 * SetVcselPulsePeriod/PerformSingleRangingMeasurement + 内部闭环），
 * 函数体原样（零改动）；裁剪清单已核（传递闭包 = 75 函数）。
 * 未裁剪/演示件（vl53l0x_test/print_pal_error/mode_string/GetPalErrorString/
 * 设备 ID 校验 GetDeviceInfo/校准缓存 AjustOK 路径/中断模式/XTalk/Offset
 * 校准/连续测量）全部范围外——manifest notes 记录。
 */

#include "vl53l0x_core.h"

/* 文件级数据（api_calibration.c 原样提取——is_aperture 依赖） */
...
/* 数据表（InterruptThresholdSettings——原 vl53l0x_interrupt_threshold_settings.h 头文件内容迁移到 .c：防多 TU 重复定义） */
...
/* 数据表（DefaultTuningSettings——原 vl53l0x_tuning.h 头文件内容迁移到 .c：防多 TU 重复定义） */
