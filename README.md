...
| 从哪拿 | 文件名（`<版本>` 形如 `v1.2.0`） | 体积 | 给谁用 |
| --- | --- | --- | --- |
| [Releases 最新版](https://github.com/AK47n/firstep/releases/latest) | `firstep-full-<版本>.zip` | 约 770 MB | **新用户（推荐）**：工具 + 五个库 + 电赛资料库（真题 / 手册 / 例程）；解压后**先看第一个文件 `00-START-HERE.txt`**（三步装好，别去猜那些 .bat） |
...
- 当前版本：**v1.2.2**（含电赛资料库；2026-09-19 发布——升级不再往你盘上写别人的库备份、不再漏发新增文件，校验失败不再留整卷残骸，持久「内容不符」半分钟内明确失败）
- 上一版：v1.2.1（2026-09-16 发布，2026-09-19 重发——补回 MSPM0（CCS）工程缺的编码设置文件，中文注释不再有乱码风险）
...
- **两级发布**：
  ...
  - **一键全量下载**（整份 firstep，约 770 MB，tag 与软件版本同号）：GitHub Releases 发布 `firstep-full-<tag>.zip`（超 1.9 GB 自动拆 `.part<N>.zip`）+ `.manifest.json`；工具内「设置 → 完整包下载」→「检查完整包」→「下载完整 firstep」→ 自动下载 / 校验 / 替换 / 重启。**适用**：修损坏、换机器、本地还没有资料库基线（无基线无法算增量）；完成后资料库基线一并写回，后续只收增量
...
- **解压报「路径太长」（错误代码 `0x80010135`）**：这是 Windows 自带解压器的老限制——**它算的是「解压到的位置 + 包里的路径」总长，上限 259 字符**，所以把包解到桌面深处（用户名一长就超）会撞上。解决办法：把 zip 解到盘根短目录，或改用系统自带的 `tar`：

  ```powershell
  mkdir C:\firstep
  tar -xf "$env:USERPROFILE\Downloads\firstep-full-<版本>.zip" -C C:\firstep
  ```

  `tar` 是 Windows 10/11 自带的，走长路径 API，不会报这个错。**千万别在那个报错框上点「跳过」**——它不会中止解压，只会静默丢掉报错的那个文件，你会拿到一个看似解压成功的残缺包。（v1.2.1 起包内路径已按这条上限压到最短，正常解压姿势不会再撞。）
...
- **一键下载完整 firstep（约 770 MB）**：打开工具 →「设置」→「完整包下载」→「检查完整包」→「下载完整 firstep」。适合修损坏 / 换机器 / 本地还没资料库基线（检查资料库更新时若提示「版本未知」，那里的按钮也会直接带你走这条路）。下载完自动替换并重启，中途取消或关页面后重试只补没下完的卷。
